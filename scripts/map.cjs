...
Promise.all([
  ...
  fileValues['TerritoryType.csv'].forEach((row) => {
    ...
    if (!id || ['offset', 'key'].includes(row[0])) {
      return
    }
    const placeName = row[6]
    const ja = fileValues['PlaceName.csv'].find(v => v[0] === placeName)?.[1]
    const en = fileValues['PlaceName_en.csv'].find(
      v => v[0] === placeName,
    )?.[1]
    const cn = fileValues['PlaceName_cn.csv'].find(
      v => v[0] === placeName,
    )?.[1]
    const souma = fileValues['PlaceName_souma.csv'].find(
      v => v[0] === placeName,
    )?.[1]
