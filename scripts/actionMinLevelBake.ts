...
import { ACTION_UPGRADE_LEVEL_OVERRIDES, ACTION_UPGRADE_STEPS } from '../src/utils/compareSaveAction'
...
function buildUpgradeChainMinLevelMap(roleActionIds: number[]) {
  ...
  function getBaseLevel(actionId: number) {
    const override = Number(ACTION_UPGRADE_LEVEL_OVERRIDES[actionId])
    if (Number.isFinite(override) && override > 0)
      return normalizeLevel(override)
...
function writeRoleActionIds(roleActionIds: number[]) {
  const lines: string[] = []
...
function writeUpgradeChainMinLevelMap(map: Record<number, number>) {
  ...
  const lines: string[] = []
  lines.push('// Generated by scripts/actionMinLevelBake.ts')
  lines.push('// Source: action2ClassJobLevel.json + ACTION_UPGRADE_STEPS + ACTION_UPGRADE_LEVEL_OVERRIDES')
