...
import { buildEnemyLine, defaultDdPath, parseEnemyLine, vulnerabilityKeys } from "./ddFormat";
...
const treasureMonsterAliases = new Set(["mimic", "quivering coffer"]);
...
function parseArgs(argv: string[]): CliOptions {
  ...
  for (const arg of argv) {
    ...
    else if (arg === "--only-undefined-to-bool") options.onlyUndefinedToBool = true;
...
async function loadBnpcNames(bnpcPath: string): Promise<Map<number, string>> {
  ...
  for (const row of rows) {
    ...
    if (!parsed || !parsed.singular) continue;
...
function parseCompendiumFile(
  fileName: string,
  content: string,
): { aliases: string[]; vulnerabilities: CompendiumVulnerabilities } | null {
  ...
  if (frontmatterName) aliases.add(canonicalizeName(frontmatterName));
  if (nickname) aliases.add(canonicalizeName(nickname));
...
function areVulnerabilitiesEqual(
  a: CompendiumVulnerabilities,
  b: CompendiumVulnerabilities,
): boolean {
  return vulnerabilityKeys.every((key) => a[key] === b[key]);
}

function areEffectiveVulnerabilitiesEqual(
  a: CompendiumVulnerabilities,
  b: CompendiumVulnerabilities,
): boolean {
...
async function buildCompendiumIndex(
  ...
  const targetDirectories = rootEntries
    .filter(
      (entry) =>
        entry.isDirectory() &&
        entry.name.startsWith(directoryPrefix) &&
        entry.name.endsWith("_enemies"),
    )
  ...
  for (const directoryPath of targetDirectories) {
    ...
    for (const fileEntry of fileEntries) {
      if (!fileEntry.isFile() || path.extname(fileEntry.name) !== ".md") continue;
...
function toDdVulnerabilities(
  compendium: CompendiumVulnerabilities,
...
function buildNextVulnerabilities(
  ...
  if (!onlyUndefinedToBool) return incoming;
  ...
  return {
    ...
    sleep:
      before.sleep === undefined && incoming.sleep !== undefined ? incoming.sleep : before.sleep,
    heavy:
      before.heavy === undefined && incoming.heavy !== undefined ? incoming.heavy : before.heavy,
...
function shouldSkipTreasureMonster(comment: string, enemyName: string): boolean {
  if (comment.includes("拟态怪") || comment.includes("宝箱怪")) return true;

  return buildNameCandidates(enemyName).some((candidate) => treasureMonsterAliases.has(candidate));
}

function extractDdVulnerabilities(
  record: EnemyRecord,
): Record<VulnerabilityKey, boolean | undefined> {
...
function collectVulnerabilityChanges(
  ...
  for (const key of vulnerabilityKeys) {
    ...
    if (previousValue === undefined) undefinedToBool.push(change);
    else boolToBool.push(change);
  }
...
async function writeReport(
  ...
  const reportLines = [
    ...
    ...(conflicts.length > 0
      ? conflicts.map(
          (conflict) =>
            `- ${conflict.alias}: ${conflict.existingFilePath} <> ${conflict.incomingFilePath}`,
        )
      : ["- 无"]),
    "",
  ];
...
function summarize(items: string[], label: string): void {
  ...
  if (items.length > 20) console.log(`  ... 其余 ${items.length - 20} 条`);
}
...
for (let index = 0; index < lines.length; index += 1) {
  ...
  if (insideEnemiesData && /^ {2}\},$/.test(line)) {
    insideEnemiesData = false;
    continue;
  }

  if (!currentMap || !insideEnemiesData) continue;
  ...
  for (const candidate of candidates) {
    ...
    if (currentIndex.ambiguous.has(candidate)) matchedAmbiguousAlias = candidate;
  }

  if (!matchRecord) {
    if (matchedAmbiguousAlias)
      ambiguousEntries.push(
        `${currentMap}:${parsed.id} (${enemyName}) 通过别名 ${matchedAmbiguousAlias} 命中冲突项`,
      );
    else unmatchedEntries.push(`${currentMap}:${parsed.id} (${enemyName})`);
    continue;
  }
...
if (nextContent !== original) await writeFile(options.ddPath, nextContent, "utf8");
...
console.log(
  `写入模式：${options.onlyUndefinedToBool ? "仅写入 undefined -> bool" : "写入所有匹配到的布尔值"}`,
);
...
summarize(
  conflicts.map(
    (conflict) => `${conflict.alias}: ${conflict.existingFilePath} <> ${conflict.incomingFilePath}`,
  ),
  "compendium 冲突别名",
);
