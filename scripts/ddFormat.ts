...
function formatValue(value: unknown): string {
  ...
  throw new Error(`Unsupported value type in dd.ts formatter: ${JSON.stringify(value)}`);
}
  ...
  for (const key of enemyPropertyOrder) {
    ...
    if (key === "vulnerabilities") {
      const vulnerabilities = normalizeVulnerabilities(
        record[key],
        options.completeVulnerabilities,
      );
      if (vulnerabilities) {
...
function formatEnemyRecord(record: EnemyRecord, options: NormalizeOptions): string {
  ...
  const segments = orderedKeys(normalized, enemyPropertyOrder).map(
    (key) => `${key}: ${formatValue(normalized[key])}`,
  );
  return `{ ${segments.join(", ")} }`;
}
...
function parseEnemyLine(line: string): ParsedEnemyLine | undefined {
  ...
  const record = new Function(`"use strict"; return (${objectLiteral});`)() as EnemyRecord;
  return {
    comment,
    id: Number(idText),
    indent,
