...
const zoneFilter = computed(
  () =>
    Object.fromEntries(
      zoneList.map((zoneId) => [zoneId, filterValue[getInstanceLengthByZoneId(zoneId)]]),
...
watchEffect(() => {
  filterConfig.value = Object.fromEntries(
    zoneList.map((zoneId) => [zoneId, filterValue[getInstanceLengthByZoneId(zoneId)][0]]),
  ) as Record<ZoneIdType, FilterType>;
});
...
function checkImmediatelyMonster() {
  ...
  const monsters = monstersData.value.filter(
    (item) => item.zoneId === playerZoneId.value && timestamp - item.timestamp < 5000,
  );
    ...
    if (hunt && rank === "A") {
      ...
      if (exist) {
        ...
        const monsters = allMonstersData.value.filter(
          (item) => item.zoneId === playerZoneId.value && item.instance === instance,
        );
        // number等于 从1、2、3、4、5、6中寻找第一个不在 monsters 中存在的number
        let number = -1;
        for (let i = 1; i <= 6; i++) {
...
function updateWaymarks() {
  ...
  const monsters = monstersData.value
    .filter((item) => item.zoneId === playerZoneId.value && item.instance === playerInstance.value)
...
function oneMapInstanceClear(zoneId: number) {
  ...
  ElMessageBox.confirm(
    `确定要清空「${Map[zoneId as ZoneIdType].name.cn}」的${instance}线的怪物吗？`,
    `${instance}线清空`,
    {
      confirmButtonText: "确定",
      cancelButtonText: "取消",
...
function oneMapClear(zoneId: ZoneIdType) {
  ElMessageBox.confirm(`要清空「${Map[zoneId as ZoneIdType].name.cn}」的怪物吗？`, "本图清空", {
    confirmButtonText: "确定",
    cancelButtonText: "取消",
    type: "warning",
  }).then(() => {
...
function oneMapInstanceExport(zoneId: number) {
  ...
  if (instance.includes("-")) {
    ...
    const data = monstersData.value.filter(
      (item) => item.zoneId === zoneId && item.instance >= min && item.instance <= max,
    );
    if (!data) {
      throw new Error("找不到该地图");
    }
    ...
    Promise.race([
      ...
      .then(() => {
        ...
        const otherGameVersion = allMonstersData.value.filter(
          (v) => getZoneGameVersion(v.zoneId as ZoneIdType) !== gameVersion.value,
        );
    ...
    Promise.race([
      ...
      .then(() => {
        allMonstersData.value = [
          ...allMonstersData.value.filter((item) => item.zoneId !== data[0]!.zoneId),
          ...data,
        ];
        mergeOverlapMonsters();
        ElMessageBox.close();
...
function importOneInstanceStr() {
  ElMessageBox.prompt("请输入要导入的字符串", "单独导入某条线", {
    ...
    const targetMonsters = monstersData.value.filter(
      (item) => item.zoneId === data[0]!.zoneId && item.instance === data[0]!.instance,
    );
    Promise.race([
      ElMessageBox.confirm(
        `要舍弃当前的数据，并替换为导入的数据吗？`,
...
function cleanUpExpiredData(): Promise<void> {
  ...
  // 无提醒的清理超过24小时的怪物数据
  allMonstersData.value = allMonstersData.value.filter(
    (item) => item.timestamp > 0 && item.timestamp > Date.now() - 1000 * 60 * 60 * 24,
  );
...
<template>
  <div :style="COLOR_STYLE">
    <h3 v-if="!wsConnected">无法连接到ACTWebSocket，找怪功能无法工作，但你可以导入导出数据。</h3>
    ...
    <el-col class="menu">
      <el-row>
        ...
        <el-button text bg @click="importOneZoneStr"> 单独导入某个图 </el-button>
        <el-button text bg m-r-5 @click="importOneInstanceStr"> 单独导入某条线 </el-button>
      </el-row>
      <el-row>
    ...
    <el-row class="map-container" flex="~ wrap" m-t-1>
      <el-row v-if="dev">
        ...
        <el-button type="warning" @click="mergeOverlapMonsters"> 合并怪物 </el-button>
    ...
    <div class="map-container" flex="~ wrap">
      <div v-for="(m, i) in zoneListUsed" :key="m" class="map-info" flex="~ col" position-relative>
        ...
        <ul class="options" position-absolute right-0 top-0 mr-2 mt-1 p0>
          <li class="option">
            <el-button size="small" w-5em @click="oneMapClear(m)"> 本图清空 </el-button>
          </li>
          <li class="option">
            <el-button size="small" w-5em @click="oneMapExport(m)"> 本图导出 </el-button>
          </li>
          <li v-show="getInstanceLengthByZoneId(m) !== 1" class="option">
            <el-button
              v-show="!(getInstanceLengthByZoneId(m) !== 1 && filterConfig[m]?.includes('-'))"
              size="small"
              w-5em
