...
function updateSearchTargets() {
  ...
  const matched = combatants.value.filter(
    (c: PluginCombatantState) => c.Name && c.Name.includes(searchTargetName.value),
  );
...
function drawRadar() {
  ...
  const primary = combatants.value.find(
    (c: PluginCombatantState) => c.Name === primaryPlayer.value,
  );
  if (!search) {
...
function drawAll() {
  ...
  const searchs = combatants.value.filter(
    (c: PluginCombatantState) => c.Name !== primaryPlayer.value && c.ID,
  );
      ...
      <div v-show="searchTargetName" class="radar-wrapper">
        <div v-if="searchTargets.length > 1">
          <button
            ...
            @click="
              currentTargetIndex =
                (currentTargetIndex - 1 + searchTargets.length) % searchTargets.length
            "
          >
            ←
          </button>
          <span> （{{ currentTargetIndex + 1 }} / {{ searchTargets.length }}） </span>
          <button
