...
function play() {
  ...
  while (nextTTSIndex < ttsLines.length) {
    ...
    if (line.time - timelineStore.configValues.ttsAdvance > runtimeTimeSeconds.value)
      break
...
function handleLogEvent(e: { detail: { logs: string[] } }) {
  ...
  for (const log of logs) {
    ...
    if (countdown) {
    ...
    else if (regexes.countdownCancel.test(log) || regexes.wipe1.test(log) || regexes.wipe2.test(log)) {
      // 取消倒计时 / 团灭 / 胜利
      stopTimeline()
      currentRuntimeSeconds = runtimeTimeSeconds.value
    }
      ...
      for (const item of lines) {
        ...
        if (!sync)
          continue
        if (item.syncOnce && item.syncAlready)
          continue
        if (currentRuntimeSeconds < item.time - item.windowBefore)
          continue
        if (currentRuntimeSeconds > item.time + item.windowAfter)
          continue
        if (!sync.test(log))
          continue
...
function syncTimeline(targetTime: number) {
  ...
  while (nextTTSIndex < loadedTimelineTTS.value.length
