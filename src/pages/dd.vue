...
import { getMaps } from "@/resources/dd";
...
const handleEnmityTargetData: EventMap["EnmityTargetData"] = (e) => {
  ...
  if (lastTime && delay > 20 && delay < 150 && elMsg) {
    elMsg.close();
  } else if (lastTime && delay < 2000 && delay > 500 && !elMsg && closeCount.value === 0) {
    elMsg = ElMessage({
      showClose: true,
...
const handleLogLine: EventMap["LogLine"] = (e) => {
  ...
  if (network6d) {
    ...
    if (["10000008", "10000009"].includes(command)) {
      const key = network6d.groups!.command! === "10000008" ? "return" : "passage";
...
watchEffect(() => {
  ...
  (state.value.data ? addOverlayListener : removeOverlayListener)("LogLine", handleLogLine);
});
      ...
      <main class="main">
        ...
        <div v-show="demo || state.tarData">
          <div class="tar-info">
            ...
            <div
              v-if="state.tarData?.vulnerabilities && (demo || state.tarData?.detect !== 'boss')"
            ...
            >
              <div
                ...
                v-show="
                  demo ||
                  (state.tarData?.vulnerabilities?.[k as keyof Abilities] !== undefined &&
                    state.partyAbilities[k as keyof Abilities])
                "
                :key="k"
          ...
          <pre>{{ demo ? $t("dd.test.note") : getLangString(state.tarData?.note ?? "") }}</pre>
        </div>
      </main>
    </div>
  </CommonActWrapper>
...
$accent-color: #947b31;
$font-family:
  "Microsoft YaHei UI", "Microsoft YaHei", "PingFang TC", "LiHei Pro", "Heiti TC", "Arial",
