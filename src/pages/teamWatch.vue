...
function getCooldownAnimationClass(skill: TeamWatchSkillView) {
  ...
  const shouldAnimate = state.isCharge ? state.charges < state.maxCharges : state.isCooling;
  if (!shouldAnimate) return "";
  if (!state.isCharge) return "normal-action-animation";
...
function openSettings() {
  window.open(
    `${import.meta.env.BASE_URL}#/teamWatchSettings`,
    "teamWatchSettings",
...
onUnmounted(() => {
  ...
  if (runtimeRaf) cancelAnimationFrame(runtimeRaf);
});
</script>
...
<template>
  <CommonActWrapper>
    <div v-if="dev || demo" class="menu-tools">
      ...
      <el-button size="small" @click="store.setFakeMode(false)"> 实际队伍 </el-button>
      <el-button size="small" @click="store.setFakeMode(true)"> 测试：模拟小队 </el-button>
      <el-button size="small" @click="store.triggerAllVisibleSkills()"> 测试：全部触发 </el-button>
      <el-button size="small" @click="store.fillResourceStates()"> 测试：填满量谱 </el-button>
    </div>
    <div class="team-watch-root">
      <main class="member-list">
        <article v-for="member in store.members" :key="member.id" class="member-row">
          <button
          ...
          >
            ...
            <span v-if="!skillState(skill).isCharge" class="recast-text">
            ...
            <span v-if="skillState(skill).extraText" class="resource-text">
              {{ skillState(skill).extraText }}
            </span>
            <span
              v-else-if="
                skillState(skill).hasResourceCost && skillState(skill).resourceValue !== undefined
              "
              class="resource-text"
