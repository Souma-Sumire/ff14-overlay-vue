...
const handleChange: UploadProps["onChange"] = async (uploadFile) => {
  ...
  for (const line of lines) {
    ...
    if (parts[0] !== "252") continue;

    const bytes = convertLogLineToBytes(parts.slice(3, -2));
    if (!includesBytes(bytes, new TextEncoder().encode("_rsv_"))) continue;

    const parsed = parseRsvBytes(bytes.slice(30));
    const key = parsed.str1.replace(/^(rsv_\d+)_[^_]+_[^_]+_[^_]+_[^_]+_([^_]{9}_[^_]+)$/, "$1_$2");
...
<template>
  <div class="min-h-screen p-6">
    <h1 class="mb-4 text-2xl font-bold">FFXIV RSV Dumper</h1>
    ...
    >
      ...
      <div class="el-upload__text">拖拽日志文件到此处或 <em>点击上传</em></div>
    </el-upload>
    ...
    <el-empty v-if="!results.length" description="请上传 act.log 文件" class="mt-6" />

    <el-table v-else :data="results" stripe style="width: 100%; margin-top: 20px" border>
