...
const handleLogEvent: EventMap["onLogEvent"] = (e) => {
  for (const log of e.detail.logs) {
    ...
    if (match?.groups) {
      ...
      logger.log(
        `[LogEvent] 捕获标点变更: ${operation} waymarkIdx:${waymarkIdx} waymarkName:${waymarkNames[waymarkIdx]} (x:${x}, y:${y}, z:${z})`,
      );
      ...
      if (xValid && yValid) {
        logger.log(
          `[LogEvent] 检测到特征信号 (WaymarkIdx: ${waymarkIdx}, Flags: 0x${flags.toString(16)})`,
        );
...
function ask() {
  const flags =
    StatusFlags.IsAnonymous |
    StatusFlags.IsIdHidden |
    StatusFlags.IsInitiator |
    StatusFlags.IsWaymarkEnabled;
