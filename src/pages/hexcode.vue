...
function getUtf8ByteLength(str: string) {
  ...
  for (let i = 0; i < str.length; i++) {
    ...
    if (charCode < 0x80) totalLength += 1;
    else if (charCode < 0x800) totalLength += 2;
    else if (charCode < 0x10000) totalLength += 3;
    else if (charCode < 0x200000) totalLength += 4;
  }
  return totalLength;
}
...
function handleReplace() {
  text.value = text.value
    .replaceAll(
      /次に実行する1回のウェポンスキルの与ダメージが(\d+)％上昇する。/g,
      "效果时间内，自身发动的1次战技的伤害提高$1%",
    )
    .replaceAll(
      /敵またはパーティメンバーひとりを対象とする。<hex:02100103>対象の目前まで素早く移動する。/g,
      "以一名敌人为目标，自身快速移动到目标身边",
    )
    .replaceAll(
      /発動条件を満たすと(.+?)のいずれかに変化する。/g,
      "满足发动条件时，会变为$1中的一个",
    )
    .replaceAll(
      "この魔法は固有のリキャストタイマーを持ち、実行すると他のウェポンスキルと魔法にこのアクションと同じリキャストタイムが発生する。",
      "该魔法有单独计算的复唱时间，发动后其他战技与魔法会进入与该战技相同的复唱时间",
    )
    .replaceAll(
    ...
    .replaceAll(
      /<hex:024804F201F803><hex:024904F201F903>追加効果：<hex:0249020103><hex:0248020103>自身に<hex:024804F201F403><hex:024904F201F503>「(.+?)」<hex:0249020103><hex:0248020103>を付与する。/g,
      "<hex:024804F201F803><hex:024904F201F903>追加效果：<hex:0249020103><hex:0248020103><hex:024804F201F403><hex:024904F201F503>$1<hex:0249020103><hex:0248020103>　　　　　　　　　　　",
    )
    ...
    .replaceAll(
      /<hex:02100103><hex:024804F201F803><hex:024904F201F903>バリア量：<hex:0249020103><hex:0248020103>(?:回復力|恢复力)(\d+)相当/g,
      "，该防护罩能够抵消相当于恢复力$1的伤害量　",
    )
    ...
    .replace(
      /^対象とその周囲の敵に([火水氷雷风土光暗無])属性範囲魔法攻撃。/,
      "对目标及其周围的敌人发动$1属性范围魔法攻击",
    )
    .replace(
      /^対象とその周囲の敵に([火水氷雷风土光暗無])属性範囲魔法攻撃。/,
      "对周围的敌人发动$1属性范围魔法攻击",
    ...
    .replaceAll(
      /2体目以降の対象への威力は(\d+)％減少する。/g,
      "攻击复数敌人时，对目标之外的敌人威力降低$1%",
    )
    .replaceAll("対象に物理攻撃。", "对目标发动物理攻击")
    ...
    .replaceAll(
      "クリティカルヒットやダイレクトヒットの発動率を上昇させる效果を受けている場合は与ダメージが上昇する。",
      "受暴击或直击发动率提高效果影响时，攻击造成的伤害提高　　　　　　　　　　　　　　　　　　　　　　　　",
    )
    // .replaceAll('の', '的')
    .replaceAll(/自身对目标造成的伤害提高(\d+)%/g, "自身造成的伤害提高$1%")
    .replaceAll(
      /自身のウェポンスキルのリキャストタイムを(\d+)[％%] ?短縮させる。/g,
      "自身的自动攻击间隔、战技与魔法的咏唱及复唱时间缩短$1%",
    )
    .replaceAll(
      /既に<hex:024804F201F403><hex:024904F201F503>「([^」]+)」<hex:0249020103><hex:0248020103>が付与されている場合は、その持续时间を(\d+)秒延長する。/g,
      "若已经附加<hex:024804F201F403><hex:024904F201F503>$1<hex:0249020103><hex:0248020103>状态，则持续时间延长$2秒",
    )
    ...
    .replaceAll(
      ...
      "消耗耐久以提高制品品质<hex:02100103>不会消耗制作力，但是成功率比较低",
    )
    .replaceAll("成功率", "成功率")
    // .replaceAll('％', '% ')
    ...
    .replaceAll(
      /次に実行する魔法のダメージと回復效果を(\d+)％上昇させる。/g,
      "下次发动魔法攻击造成的伤害和恢复效果提高$1%",
    )
    .replaceAll(
      /最大スタック数：<hex:0249020103><hex:0248020103>(\d)/g,
      "最大档数：<hex:0249020103><hex:0248020103>$1　　　",
    )
    .replaceAll(
      /最大チャージ数：<hex:0249020103><hex:0248020103>(\d)/g,
      "积蓄次数：<hex:0249020103><hex:0248020103>$1　　　",
    )
...
<template>
  <main>
    ...
    <button @click="handleReplace">一键替换日文</button>
    <article>
      {{ message.every((m) => m.passed) ? "02开头指令全部通过" : "可能有错误" }}
      <ul>
        <li v-for="(m, i) in message" :key="i">
          <span :style="{ color: m.passed ? 'black' : 'red' }">{{
            `${m.passed ? "通过" : `错误`} ${m.msg} ${m.full}`
          }}</span>
        </li>
      </ul>
    </article>
