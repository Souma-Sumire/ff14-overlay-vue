...
import { ElButton, ElInput, ElInputNumber, ElPopconfirm, ElSwitch } from "element-plus";
...
const data = reactive([
  ...
  },
  {
    id: "tr3",
    cd: computed(() => {
      return 30 - Math.floor((now.value - (game.status.get("A85") ?? 0)) / 1000);
    }),
    style: {
  ...
  },
  {
    id: "tr4",
    cd: computed(() => {
      return 30 - Math.floor((now.value - (game.status.get("A86") ?? 0)) / 1000);
    }),
    style: {
  ...
  },
  {
    id: "fl3",
    cd: computed(() => {
      return 30 - Math.floor((now.value - (game.status.get("BC9") ?? 0)) / 1000);
    }),
    style: {
  ...
  },
  {
    id: "fl4",
    cd: computed(() => {
      return 30 - Math.floor((now.value - (game.status.get("BCA") ?? 0)) / 1000);
    }),
    style: {
...
<template>
  <div class="dnc-overlay">
    <header v-show="demo" class="settings">
      <form>
        连击X:
        <el-input-number v-model="settings.combo.left" class="input input-number" size="small" />
      </form>
      <form>
        连击Y:
        <el-input-number v-model="settings.combo.top" class="input input-number" size="small" />
      </form>
      <form>
        触发3X:
        <el-input-number v-model="settings.tr3.left" class="input input-number" size="small" />
      </form>
      <form>
        触发3Y:
        <el-input-number v-model="settings.tr3.top" class="input input-number" size="small" />
      </form>
      <form>
        触发4X:
        <el-input-number v-model="settings.tr4.left" class="input input-number" size="small" />
      </form>
      <form>
        触发4Y:
        <el-input-number v-model="settings.tr4.top" class="input input-number" size="small" />
      </form>
      <form>
        百花X:
        <el-input-number v-model="settings.fl.left" class="input input-number" size="small" />
      </form>
      <form>
        百花Y:
        <el-input-number v-model="settings.fl.top" class="input input-number" size="small" />
      </form>
      <form>
        剩余警告:
        <el-input-number
      ...
      <form>
        常规颜色:
        <el-input v-model="settings.color" class="input input-string" size="small" />
      </form>
      <form>
        常规阴影:
        <el-input v-model="settings.shadow" class="input input-string" size="small" />
      </form>
      <form>
        警告颜色:
        <el-input v-model="settings.warncolor" class="input input-string" size="small" />
      </form>
      <form>
        警告阴影:
        <el-input v-model="settings.warnshadow" class="input input-string" size="small" />
      </form>
      <form>
        零值文本:
        <el-input v-model="settings.zeroStr" class="input input-string" size="small" />
      </form>
      <form>
        字号:
        <el-input-number v-model="settings.size" class="input input-number" size="small" />
      </form>
      <form>
        字体:
        <el-input v-model="settings.fontFamily" class="input input-string" size="small" />
      </form>
      ...
      <br />
      <form>
        <el-popconfirm :teleported="false" title="确定要重置？" @confirm="resetSettings">
          <template #reference>
    ...
    <main>
      <div v-for="item in data" :key="item.id">
        <span :class="item.cd <= settings.warn && !demo ? 'warning' : ''" :style="item.style">
