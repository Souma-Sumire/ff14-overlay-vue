...
import { getCactbotLocaleMessage, getLocaleMessage } from "@/composables/useLang";
...
const applyRealtimeLocalWaymark = useDebounceFn(async (place: MacroInfoPlace["Place"]) => {
  if (!realtimeLocalMode.value) return;
  try {
...
function cleanDutyHtml(rawHtml: string): string {
  ...
  if (isFullPage) {
    ...
    const article = fullDoc.querySelector(
      "#root > div.main-container > div > div > main > div.content-container > article",
    );
    if (!article) return "";
    doc = article;
  } else {
...
watch(dutyContent, async (newVal) => {
  ...
  if (!newVal || dutyStatus.value === "success" || dutyStatus.value === "empty") return;
...
function handleRealtimeCoordinateChange(macro: MacroInfoPlace) {
  if (!realtimeLocalMode.value || !macro.Editable) return;
  ...
  <el-container v-show="macroStore.show" class="elcontainer">
    <el-header height="auto" class="elheader">
      ...
      <div class="header-row">
        ...
        <el-button size="small" color="#BA5783" @click="macroStore.importPPJSON()">
      ...
      <el-space wrap alignment="flex-start" class="macro-grid">
        ...
        >
          <div class="badge-group">
            ...
            <span v-else class="subtle-badge from-native">{{ $t("zoneMacro.badgeNative") }}</span>
          </div>
          ...
          <div v-if="'Place' in macro">
            <el-space v-show="macro.Editable">
              <el-table
                ...
                size="small"
              >
                <el-table-column align="center" :label="$t('zoneMacro.tableActive')" width="50">
                ...
                <el-table-column align="center" :label="$t('zoneMacro.tableMark')" width="50">
                  <template #default="scope">
                    <span>{{ scope.row[0] }}</span>
                  </template>
                </el-table-column>

                <el-table-column align="center" :label="$t('zoneMacro.tableX')" width="120">
                  <template #default="scope">
                ...
                <el-table-column align="center" :label="$t('zoneMacro.tableZ')" width="120">
                  <template #default="scope">
                ...
                <el-table-column align="center" :label="$t('zoneMacro.tableY')" width="120">
                  <template #default="scope">
        ...
        >
          <div v-if="dutyStatus === 'idle'" class="duty-placeholder">
            <el-button type="primary" plain @click="() => loadDuty(false)"> 加载攻略 </el-button>
          </div>
          ...
          <div v-else-if="dutyStatus === 'error'" class="duty-placeholder">
            ...
            <el-button size="small" type="primary" @click="() => loadDuty(true)"> 重试 </el-button>
          </div>
...
$main-font:
  "FFXIV", "Helvetica Neue", Helvetica, "PingFang SC", "Hiragino Sans GB", "Microsoft YaHei",
