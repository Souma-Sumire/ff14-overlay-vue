...
function resetHotbar(standard: boolean, technical: boolean) {
  if (playerJob === "DNC") {
    if (standard)
      doTextCommand(
        `/hotbar set ${ending[config.value.lang].standard} ${config.value.hotbar.standard.join(" ")}`,
      );
    if (technical)
      doTextCommand(
        `/hotbar set ${ending[config.value.lang].technical} ${config.value.hotbar.technical.join(" ")}`,
      );
    lastStep = -1;
  }
}
  ...
  countdown: (time: number) => {
    ...
    if (config.value.cdDancingStart) {
      doQueueActions([
        ...
        {
          c: "DoTextCommand",
          p: `/ac ${ending[config.value.lang].standard}`,
          d: Math.max(0, (time - 15) * 1000),
        },
      ]);
    }
...
onMounted(() => {
  addOverlayListener("onPlayerChangedEvent", (e) => {
    ...
    if (playerID !== e.detail.id) {
      ...
      netRegex.losesEffect = NetRegexes.losesEffect({
        targetId: playerID.toString(16).toUpperCase(),
        effectId: "71[AB]",
      });
    }
    if (!(e.detail.job === "DNC")) return;
    const curStep = e.detail.jobDetail?.currentStep;
    const needsSteps = e.detail.jobDetail?.steps;
    if (curStep === undefined || needsSteps === undefined || curStep === lastStep) return;

    if (needsSteps.length && curStep < needsSteps.length) {
      const stepAction =
        step[config.value.lang][
          ["Emboite", "Entrechat", "Jete", "Pirouette"].indexOf(needsSteps[curStep]!)
        ];
      doTextCommand(
        `/hotbar set ${stepAction} ${config.value.hotbar[needsSteps.length === 2 ? "standard" : "technical"].join(" ")}`,
      );
      const currentStep = e.detail.jobDetail?.currentStep;
      if (
        combatState.inGameCombat === false &&
        combatState &&
        currentStep !== undefined &&
        config.value.autoDance
      ) {
    ...
    <div bg-white p-l-3>
      <el-form label-width="9rem" style="max-width: 300px">
        ...
        <el-form-item label="标准舞步热键栏">
          <el-input
            v-model="config.hotbar.standard[0]"
            type="number"
            style="width: 2.5rem"
            min="1"
            max="10"
          ...
          />
          <el-input
            v-model="config.hotbar.standard[1]"
            type="number"
            style="width: 2.5rem"
            min="1"
            max="12"
          ...
          <el-popover
            trigger="hover"
            content="标准舞步的热键栏位置，例如2-1，代表热键栏2的编号1位置。"
          >
            <template #reference>
              <svg
                t="1717309838763"
        ...
        <el-form-item label="技巧舞步热键栏">
          <el-input
            v-model="config.hotbar.technical[0]"
            type="number"
            style="width: 2.5rem"
            min="1"
            max="10"
          ...
          />
          <el-input
            v-model="config.hotbar.technical[1]"
            type="number"
            style="width: 2.5rem"
            min="1"
            max="12"
          ...
          <el-popover
            trigger="hover"
            content="技巧舞步的热键栏位置，例如2-2，代表热键栏2的编号2位置。"
          >
            <template #reference>
              <svg
                t="1717309838763"
