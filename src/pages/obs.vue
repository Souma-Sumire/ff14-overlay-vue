...
const password = useStorage("obs-passwd", '');
...
<template>
  <header v-show="showHeader && status.connected">
    <div>
      <i class="vxe-icon-dot icon"
        :style="{ color: status.recording ? 'red' : 'gray', textShadow: '0px  0px 3px black', margin: '1px' }"></i>
      <vxe-button class="btns" icon="vxe-icon-caret-right" v-show="!status.recording"
        @click="start('manual', false, true)"></vxe-button>
      <vxe-button class="btns" icon="vxe-icon-close" v-show="status.recording" @click="stop('manual')"
        size="mini"></vxe-button>
      <vxe-button class="btns" icon="vxe-icon-cut" v-show="status.recording" @click="start('manual', true, true)"
        size="mini"></vxe-button>
      <vxe-button class="btns settings" icon="vxe-icon-setting" @click="toggleWindow('settings')"
        size="mini"></vxe-button>
    </div>
  </header>
  <main v-show="showSettings">
    <vxe-form :data="config" :collapseStatus="!showMore" custom-layout size="mini">
      <vxe-form-item span="24" v-show="showSettings" class-name="hidePageBtn">
        <vxe-button size="mini" content="隐藏页面" icon="vxe-icon-eye-fill-close"
          @click="toggleWindow('header')"></vxe-button>
      </vxe-form-item>
      <vxe-form-item span="24" field="host" title="主机">
        <vxe-input v-model="config.host" size="small" placeholder="127.0.0.1:4455"
          style="width: 70%; margin-right: -5px"></vxe-input>
        <vxe-button content="帮助" size="mini" icon="vxe-icon-question" type="text" status="info"
          @click="VXETable.modal.message({ content: '先点击ACT，再点击悬浮窗，即可正常输入' })"></vxe-button>
      </vxe-form-item>
      ...
      <vxe-form-item span="24" field="greaterThan" title="小队多于">
        <vxe-input v-model="config.greaterThan" size="small" type="integer" min="0" max="8"
          style="width: 3rem"></vxe-input>
        人自动录制（8:永不自动）
      </vxe-form-item>
      ...
      <vxe-form-item span="24" title="默认" folding class-name="recPaths">
        <vxe-input v-model="config.recFilePaths.default" size="small" placeholder="留空则使用OBS录像目录"
          spellcheck="false"></vxe-input>
      </vxe-form-item>
      <vxe-form-item span="24" title="高难" folding class-name="recPaths">
        <vxe-input v-model="config.recFilePaths.ultimateOrRaidOrTrials" size="small" placeholder="绝&零式&极神"
          spellcheck="false"></vxe-input>
      </vxe-form-item>
      <vxe-form-item span="24" title="深层" folding class-name="recPaths">
        <vxe-input v-model="config.recFilePaths.vcOrDeepDungeons" size="small" placeholder="深层&异闻迷宫"
          spellcheck="false"></vxe-input>
      </vxe-form-item>
      ...
      <vxe-form-item span="24" collapse-node>
        ...
        <vxe-button v-show="!status.connected" type="text" status="danger" size="mini" icon="vxe-icon-warning-circle"
          content="未连接"></vxe-button>
        <vxe-button v-show="status.connected" type="text" status="success" size="mini" icon="vxe-icon-success-circle"
          content="已连接"></vxe-button>
      </vxe-form-item>
    </vxe-form>
  </main>
</template>
