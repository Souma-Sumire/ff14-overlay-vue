...
function ensureOverrideTargetSelection() {
  if (logImportMode.value !== 'actions-override-current')
    return
  if (selectedOverrideSheetId.value && eligibleOverrideSheets.value.some(s => s.id === selectedOverrideSheetId.value)) {
    return
  }
  selectedOverrideSheetId.value = eligibleOverrideSheets.value.find(s => s.isActive)?.id
...
const jobSkillFilters = useStorage<Record<number, number[]>>('mitigation-job-skill-filters', DEFAULT_JOB_SKILL_FILTERS, localStorage, { shallow: true })
const jobSkillOrders = useStorage<Record<number, number[]>>('mitigation-job-skill-orders', {}, localStorage, { shallow: true })
const autoArrangeToleranceSeconds = useStorage('mitigation-auto-arrange-tolerance-seconds', 2)
const damageNumberDisplayUnit = useStorage<DamageNumberDisplayUnit>('mitigation-damage-number-display-unit', 'w')
...
const skeletonPartyColumnWidths = computed(() => {
  return skeletonPartySkillCounts.value.map(skillCount => getMitigationGridColumnWidth(skillCount))
})

const jobOrder = new Map(DEFAULT_JOB_SORT_ORDER.map((job, index) => [job, index]))
const mitigationJobEnums = new Set<number>(
  mitigationKeigennSkills.flatMap(skill => skill.job.map(job => Util.baseJobEnumConverted(job))),
)
...
const mechanicOptions = computed(() => {
  ...
  return Array.from(map.entries())
    .map(([actionId, info]) => ({ actionId, action: info.action, timestamp: info.timestamp }))
...
// 当核心输入变化时运行模拟
watch([rawRows, playerActions, columns, filterMechanics], () => {
  if (isRestoringSheet.value || isHydrating.value || isSimulatingInternal)
    return
...
async function restoreFromDB() {
  ...
  try {
    const [sheetOrderItem, sheetMetaMapItem, sheetMechanicsMapItem, sheetPlannerMapItem, activeItem, mechanicAliasesItem] = await Promise.all([
      db.get('sheetOrder'),
      db.get('sheetMetaMap'),
    ...
    if (sheetOrderItem?.value && sheetMetaMapItem?.value && sheetMechanicsMapItem?.value && sheetPlannerMapItem?.value) {
    ...
    if (activeItem?.value !== undefined)
      activeSheetId.value = activeItem.value
    if (mechanicAliasesItem?.value)
      mechanicAliases.value = mechanicAliasesItem.value
...
watch(mechanicColors, () => {
  void persistMechanicColors()
}, { deep: true })

watch(sheets, () => {
  if (persistSheetsTimer !== null)
    window.clearTimeout(persistSheetsTimer)
...
watch(activeSheetId, () => {
  if (persistActiveIdTimer !== null)
    window.clearTimeout(persistActiveIdTimer)
...
watch([logImportMode, eligibleOverrideSheets], () => {
  ensureOverrideTargetSelection()
}, { deep: false })

// Sync back to sheets
watch([rawRows, playerActions, columns, playerLevel, filterMechanics], () => {
  if (isRestoringSheet.value || !activeSheetId.value || isHydrating.value)
    return
...
function applySheetToWorkspace(id: string | null) {
  ...
  void nextTick(() => {
    if (restoreToken !== sheetRestoreToken)
      return
...
function handleEncounterSelect(encounter: EncounterCandidate) {
  ...
  if ((encounter.zoneId === null || eligibleOverrideSheets.value.length === 0) && logImportMode.value === 'actions-override-current')
...
function createNewSheet(encounter: EncounterCandidate, mode: Extract<LogImportMode, 'mechanics-empty' | 'mechanics-auto'>) {
  const result = extractMechanics(encounter, {
...
function applyEncounterActionsToSheet(encounter: EncounterCandidate, sheetId: string) {
  ...
  const normalizedIncomingActions = ensurePlayerActionsRowKey(incomingActions, sheet.mechanics.rows || [])
  const prevSheets = deepClone(sheets.value)
  const prevPlayerActions = deepClone(playerActions.value)
  const nextSheets = sheets.value.map(s => s.id === sheetId
    ? {
        ...s,
        planner: {
          ...s.planner,
  ...
  runCommand({
    ...
    undo: () => {
      sheets.value = deepClone(prevSheets)
      if (shouldSyncCurrentPlayerActions)
        playerActions.value = deepClone(prevPlayerActions)
    },
  })
...
function deleteSheet(id: string) {
  ...
  if (nextSheets.length === sheets.value.length)
    return
  const prevSheets = deepClone(sheets.value)
  const prevActiveId = activeSheetId.value
  const nextActiveId = activeSheetId.value === id
    ? (nextSheets.length > 0 ? (nextSheets[0]?.id || null) : null)
    : activeSheetId.value
  runCommand({
    label: 'delete-sheet',
    redo: () => {
...
function confirmRename() {
  ...
  if (name) {
    ...
    applyWithCommand(sheets.value, nextSheets, value => (sheets.value = value), { label: 'rename-sheet' })
  }
...
function handleSheetTabClick(sheetId: string) {
  closeSheetContextMenu()
  applyWithCommand(activeSheetId.value, sheetId, value => (activeSheetId.value = value), { label: 'switch-sheet' })
}

function closeSheetContextMenu() {
  if (!sheetContextMenu.value.visible)
    return
...
function handleGlobalPointerDown(event: PointerEvent) {
  ...
  if (target?.closest('.sheet-context-menu'))
    return
  closeSheetContextMenu()
}
...
function handlePlayerLevelUpdate(value: number | undefined) {
  ...
  const changed = prevLevel !== normalized
...
function normalizeColumnsWithJobFilters(
  list: ColumnDef[],
  options: { forceGlobal?: boolean, jobFilterMap?: Record<number, number[]>, jobOrderMap?: Record<number, number[]> } = {},
) {
  ...
  if (!options.forceGlobal) {
    list.forEach((col) => {
      if (typeof col.jobEnum === 'number')
        nextMap[col.jobEnum] = Array.from(new Set(col.hiddenSkillIds || []))
    })
  }
...
function buildMechanicsExportData() {
  ...
  const rows = rawRows.value.map((r) => {
    ...
    if (r.damageType === 'physics')
      ty = 0
    else if (r.damageType === 'magic')
      ty = 1
    else if (r.damageType === 'darkness')
      ty = 2
...
function buildPlayerActionsExportData() {
  ...
  const anonSourcesList = Array.from(sourceToAnonMap.values())
  ...
  playerActions.value.forEach((a) => {
    ...
    if (!dataMap[sIdx])
      dataMap[sIdx] = {}
    if (!dataMap[sIdx][a.skillId])
      dataMap[sIdx][a.skillId] = []
  ...
  const cols = columns.value.filter(c => sourceToAnonMap.has(c.key)).map(c => ({
...
function importMechanicData(data: MitigationMechanicsImportData) {
  ...
  const importedRows: MitigationRow[] = data.rows.map((r) => {
    ...
    return {
      ...
      rawDamage: r.v,
      targetCount: typeof r.tc === 'number' && Number.isFinite(r.tc) ? Math.max(1, Math.round(r.tc)) : 1,
      ...
      castTime: typeof r.ct === 'number' && Number.isFinite(r.ct) && r.ct > 0
        ? String(r.ct)
        : undefined,
      castStartTime: typeof r.ct === 'number' && Number.isFinite(r.ct) && r.ct > 0
        ? r.t - r.ct
        : undefined,
      source: 'Import',
...
function importPlayerActionsData(data: MitigationPlayerActionsImportData) {
  ...
  const nextOrders = importedColumnsPayload ? importedColumnsPayload.nextOrders : deepClone(jobSkillOrders.value)
  const nextFilters = importedColumnsPayload ? importedColumnsPayload.nextFilters : deepClone(jobSkillFilters.value)
  const changed = !isSameState(prevActions, nextActions)
    || !isSameState(prevColumns, nextColumns)
    || !isSameState(prevOrders, nextOrders)
    || !isSameState(prevFilters, nextFilters)
    || importedMapSheetUpdate.changed
...
function promptImportDataWithOptions(options: { ensureSheetName?: string } = {}) {
  ElMessageBox.prompt('请粘贴导出的字符串', '导入', {
    ...
    if (options.ensureSheetName)
      await ensureActiveSheetReady(options.ensureSheetName)
  ...
  <div class="app-root" @contextmenu.prevent>
    ...
    <header class="toolbar">
      <div class="tool-left">
        ...
        <div v-if="activeSheet || isHydrating" class="action-btns">
          <template v-if="activeSheet">
            <el-button size="small" :disabled="!canUndo" @click="undoActions">
              撤销
            </el-button>
            <el-button size="small" :disabled="!canRedo" @click="redoActions">
              重做
            </el-button>
            ...
            <el-dropdown trigger="click">
              ...
              <template #dropdown>
                <el-dropdown-menu>
                  <el-dropdown-item @click="exportMechanics">
                    仅导出机制信息
                  </el-dropdown-item>
            ...
            <el-button size="small" :icon="Upload" @click="promptImportData">
              导入
            </el-button>
          </template>
          <template v-else>
      ...
      <div v-if="activeSheet || isHydrating" class="tool-right">
        <template v-if="activeSheet">
          ...
          <div class="stats">
            {{ filteredRows.length }} / {{ rawRows.length }} rows
          </div>

          <div class="settings-entry">
            <el-popover placement="bottom-end" :width="260" trigger="click" popper-class="config-popover">
              <template #reference>
              ...
              <div class="config-panel">
                ...
                <div class="config-item">
                  ...
                  >
                    <el-radio-button value="k">
                      千(K)
                    </el-radio-button>
                    <el-radio-button value="w">
                      万(W)
                    </el-radio-button>
                  </el-radio-group>
                </div>
          ...
          <div class="loading-party-header">
            ...
            >
              ...
              <div class="loading-skill-icons">
                <span v-for="k in skillCount" :key="`hdr-${index}-${k}`" class="skel-shimmer skel-icon" />
              </div>
            </div>
          </div>
        </div>
        <div class="loading-grid-body">
          <div v-for="row in HYDRATING_SKELETON_ROW_COUNT" :key="`row-${row}`" class="loading-grid-row">
            <div class="loading-fixed-cols">
              <div
                v-for="(width, index) in skeletonFixedCellWidths"
    ...
    <footer v-if="sheets.length > 0" class="sheet-footer">
      <div class="sheet-strip">
        ...
        <button type="button" class="sheet-add-btn" title="新建空白 Sheet" @click="createEmptySheet">
          +
        </button>
      </div>
    </footer>
    ...
    <transition name="sheet-menu-fade">
      <div
        ...
        @contextmenu.prevent
      >
        <button class="sheet-context-item" type="button" @click="handleSheetContextAction('rename')">
          重命名
        </button>
    ...
    <el-dialog v-model="showEncounterDialog" title="选择战斗记录" width="500px">
      <div class="enc-list">
        <div v-for="enc in encounters" :key="enc.id" class="enc-item" @click="handleEncounterSelect(enc)">
          <div class="enc-main">
    ...
    <el-dialog v-model="showImportModeDialog" title="导入模式" width="560px" @close="cancelImportMode">
      <div class="import-mode-panel">
        <div class="import-enc-title">
          {{ selectedEncounter?.zoneName }} <span v-if="selectedEncounter">({{ selectedEncounter?.durationStr }})</span>
        </div>
        <el-radio-group v-model="logImportMode" class="import-mode-group">
          <el-radio value="mechanics-empty">
            只生成机制名称 + 空表
          </el-radio>
          <el-radio value="mechanics-auto">
            机制名称 + 自动勾选单元格
          </el-radio>
          <el-radio value="actions-override-current" :disabled="!selectedEncounter || selectedEncounter.zoneId === null || eligibleOverrideSheets.length === 0">
            只解析释放信息并覆盖符合条件的 Sheet（地图ID一致）
          </el-radio>
        </el-radio-group>
        <div v-if="logImportMode === 'actions-override-current'" class="override-sheet-panel">
          <div class="override-sheet-title">
            请选择要覆盖的 Sheet
          </div>
          <el-radio-group v-if="eligibleOverrideSheets.length > 0" v-model="selectedOverrideSheetId" class="override-sheet-group">
      ...
      <template #footer>
        <el-button @click="cancelImportMode">
          取消
        </el-button>
...
.app-root { height: 100vh; display: flex; flex-direction: column; background: #fff; overflow: hidden; }

.toolbar {
  height: 40px; border-bottom: 1px solid #ddd; padding: 0 12px;
  display: flex; align-items: center; justify-content: space-between;
  background: #fdfdfd;
}

.tool-left { display: flex; align-items: center; gap: 12px; }
.action-btns { display: flex; gap: 8px; margin-left: 12px; align-items: center; }
.divider { width: 1px; height: 16px; background: #eee; margin: 0 4px; }

.logo { font-size: 14px; font-weight: bold; margin: 0; color: #7c3aed; }
...
.w-96 { width: 96px; }
.w-140 { width: 140px; }
.lv-box { display: flex; align-items: center; gap: 4px; }
.stats { color: #888; }

.content { flex: 1; overflow: hidden; position: relative; border-bottom: 1px solid #ddd; }
...
.sheet-tab {
  height: 28px; background: #e0e0e0; color: #666; border: 1px solid #ccc;
  border-bottom: none; border-radius: 4px 4px 0 0; padding: 0 8px;
  display: flex; align-items: center; gap: 8px; cursor: pointer;
  max-width: 300px; font-size: 12px; transition: all 0.2s;
  user-select: none;

  .sheet-name { white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }

  &.active {
    background: #fff; color: #7c3aed; border-color: #ddd; border-bottom: 1px solid #fff;
    margin-bottom: -1px; z-index: 1; height: 29px; border-top: 2px solid #7c3aed;
  }
  ...
  &:hover:not(.active) { background: #e8e8e8; }

  .close-btn {
    font-size: 10px; border-radius: 50%; padding: 2px;
...
.file-btn-mini {
  position: relative; height: 28px; display: flex; align-items: center; padding: 0 8px;
  .file-input { position: absolute; inset: 0; opacity: 0; cursor: pointer; }
}
...
.loader-content {
  background: #333; color: #fff; padding: 12px 24px; border-radius: 4px; font-size: 14px;
}

:deep(.el-icon) { vertical-align: middle; }

.enc-list { max-height: 400px; overflow-y: auto; }
.enc-item {
  padding: 10px 12px; border-bottom: 1px solid #eee; cursor: pointer;
  &:hover { background: #f5f7fa; }
}
.enc-main { display: flex; justify-content: space-between; margin-bottom: 4px; }
.enc-zone { font-weight: bold; font-size: 14px; color: #333; }
.enc-dur { color: #7c3aed; font-weight: 500; font-size: 12px; }
.enc-sub { color: #888; font-size: 11px; }
