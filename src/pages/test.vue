...
const userConfig = ref<Options & { CactbotUserDirectory: string; DefaultAlertOutput: string }>({
  ParserLanguage: "",
  ShortLocale: "",
  DisplayLanguage: "",
...
const onLogLine: EventMap["LogLine"] = (ev) => {
  ...
  for (const key in overlayPluginLogRegexes.value) {
    const regex =
      overlayPluginLogRegexes.value[key as keyof typeof overlayPluginLogRegexes.value].regex;
    if (regex.test(ev.rawLine)) {
              ...
              <div class="info-table">
                <div
                  v-for="(value, label) in {
                    [t('actSelfTest.configSection.actVersion')]: userConfig.SystemInfo.actVersion,
                    ...
                    [t('actSelfTest.configSection.parserLanguage')]: userConfig.ParserLanguage,
                    [t('actSelfTest.configSection.displayLanguage')]: userConfig.DisplayLanguage,
                    [t('actSelfTest.configSection.gameRegion')]: userConfig.SystemInfo.gameRegion,
                    [t('actSelfTest.configSection.defaultAlertOutput')]:
                      userConfig.DefaultAlertOutput,
                    [t('actSelfTest.configSection.userDirectory')]: userConfig.CactbotUserDirectory,
                  }"
                  :key="label"
                  class="info-row"
                >
            ...
            <el-card class="event-card" :class="eventEventRes[key] ? 'success' : 'danger'">
              <div class="card-title">
                ...
                <el-tag :type="eventEventRes[key] ? 'success' : 'danger'" size="small">
                  {{ eventEventRes[key] ? t("actSelfTest.success") : t("actSelfTest.untriggered") }}
                </el-tag>
              </div>
