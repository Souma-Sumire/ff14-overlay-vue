...
function extractFieldByIndex(line: string, fieldIndex: number): string {
  ...
  if (end === -1) return line.slice(start);
...
function parsePartyListLineAndAppend(
  ...
  if (parts.length < 4 || parts[0] !== "11") return;

  const partyCount = Number.parseInt(parts[2] || "", 10);
  if (!Number.isFinite(partyCount) || partyCount <= 0) return;
  ...
  for (let i = 0; i < idCount; i++) {
    ...
    if (knownName) seenPlayers.add(knownName);
  }
}
...
function parseAddCombatantLineAndAppend(
  ...
  if (id.startsWith("10")) combatantNameById.set(id, name);

  if (partyIds.has(id)) seenPlayers.add(name);
}

function extractCoreFields(
  line: string,
): { timestampStr: string; code: string; message: string } | null {
  ...
  if (!timestampStr || !code || !message) return null;
...
function parseLineAndAppend(
  ...
  if (isObtainCode(code)) {
    ...
    if (pendingRolls) tempRolls.delete(item);
  ...
  if (isRollCode(code)) {
    ...
    if (rollMatch) {
      ...
      if (!tempRolls.has(item)) tempRolls.set(item, []);
...
function parseTextAndAppend(text: string, accumulator: ParseAccumulator) {
...
function parseChunkBytesAndAppend(
  ...
  if (lineStart >= merged.length) return;

  if (flushTail) {
    let tailEnd = merged.length;
    if (tailEnd > lineStart && merged[tailEnd - 1] === 0x0d) tailEnd--;
...
globalThis.onmessage = (
  e: MessageEvent<string | WorkerInput | StreamStartInput | StreamChunkInput | StreamEndInput>,
) => {
  ...
  if (typeof payload === "object" && payload && "type" in payload) {
    ...
    if (payload.type === "stream-chunk") {
      ...
      if (!session) throw new Error(`Unknown parser stream session: ${payload.sessionId}`);
  ...
  const locale =
    typeof payload === "string"
      ? DEFAULT_LOOT_PARSER_LOCALE
      : payload.locale || DEFAULT_LOOT_PARSER_LOCALE;
