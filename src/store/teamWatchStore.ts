...
import {
  ...
  encodeBase64Payload,
  ensureRuntime,
  resolveTeamWatchSkillState,
  resolveTrackedActionId,
  updateRuntimeCollection,
  useRuntime,
} from "@/store/teamWatchHelpers";
...
const useTeamWatchStore = defineStore("teamWatch", () => {
  ...
  const watchJobsActionsIDUser = ref<Record<number, number[]>>(
    Object.fromEntries(
      Object.entries(storage.watchJobsActionsIDUser).map(([k, v]) => [Number(k), [...v]]),
    ),
  );
  ...
  function getActionMetaRaw(actionId: number, autoFetch = true) {
    ...
    if (autoFetch && actionId > 0) triggerAutoFetchActionMeta(actionId);
    return raw;
  }

  function triggerAutoFetchActionMeta(actionId: number) {
    if (actionId <= 0 || (actionMetaUser.value[actionId]?.actionCategory ?? 0) > 0) return;
    if (autoFetchMetaRequested.has(actionId)) return;

    autoFetchMetaRequested.add(actionId);
    void fetchActionMetaDraft(actionId, true).finally(() => {
  ...
  function resolveActionMeta(
    actionId: number,
    level: number,
    rawOverride?: TeamWatchActionMetaRaw,
  ) {
    const raw = rawOverride
      ? normalizeTeamWatchActionMetaRaw(actionId, rawOverride)
      : getActionMetaRaw(actionId, false);
    const configuredActionId = Math.trunc(Number(raw.id)) || actionId;
    const resolvedActionId =
      resolveUpgradeActionIdForLevel(configuredActionId, level) || configuredActionId;
  ...
  async function fetchActionMetaDraft(
    actionId: number,
    force = false,
  ): Promise<TeamWatchActionMetaRaw> {
    if (actionId <= 0) return normalizeTeamWatchActionMetaRaw(0, {});

    if (!force && actionMetaUser.value[actionId]) return getActionMetaRaw(actionId);
    ...
    const task = (async () => {
      ...
      try {
        ...
        if (!existing) saveActionMetaRaw(actionId, merged);
        else if (
          Number(existing.actionCategory ?? 0) <= 0 &&
          Number(merged.actionCategory ?? 0) > 0
        )
        ...
        if (!actionMetaUser.value[actionId]) saveActionMetaRaw(actionId, merged);
        return merged;
      }
    })();
    ...
    return fakeMode.value
      ...
      : [...party.value]
          .sort((a, b) => {
            ...
            if (aSort === bSort) return Number.parseInt(b.id, 16) - Number.parseInt(a.id, 16);
            return aSort - bSort;
          })
          .slice(0, 8);
  }
  ...
  function updateResourceManagerJobs() {
    ...
    partySource.forEach((member: Party) => {
      const normalizedJob = Util.baseJobEnumConverted(member.job);
      const normalizedId = String(member.id).toUpperCase();
      if (!partyInfo.has(normalizedJob)) partyInfo.set(normalizedJob, []);
      const ids = partyInfo.get(normalizedJob)!;
      if (!ids.includes(normalizedId)) ids.push(normalizedId);
    });
  ...
  function resolveMemberWatchActionIds(job: number) {
    ...
    if (jobId <= 0) return [...TEAM_WATCH_EMPTY_ACTIONS];

    const advancedJob = Util.baseJobEnumConverted(jobId);
    const isBase = advancedJob !== jobId;

    const source =
      watchJobsActionsIDUser.value[isBase ? advancedJob : jobId] ?? TEAM_WATCH_EMPTY_ACTIONS;
    const validSource =
      Array.isArray(source) && source.length > 0 ? source : [...TEAM_WATCH_EMPTY_ACTIONS];
  ...
  function rebuildMembers() {
    ...
    for (const member of sorted) {
      ...
      const skills: TeamWatchSkillView[] = actionIds.map((rawActionId) => {
        if (rawActionId > 0) pendingAutoFetchActionIds.add(rawActionId);
    ...
    for (const key of Object.keys(runtimeByKey)) {
      if (!activeRuntimeKeys.has(key)) Reflect.deleteProperty(runtimeByKey, key);
    }
    ...
    for (const ownerId of Object.keys(cooldownTracker)) {
      ...
      for (const rawActionId of Object.keys(ownerMap)) {
        ...
        if (!activeActions.has(actionId)) Reflect.deleteProperty(ownerMap, rawActionId);
      }
      if (Object.keys(ownerMap).length === 0) Reflect.deleteProperty(cooldownTracker, ownerId);
    }

    for (const key of Object.keys(actionLookup)) Reflect.deleteProperty(actionLookup, key);
  ...
  function triggerAllVisibleSkills() {
    ...
    members.value.forEach((member) => {
      member.skills.forEach((skill) => {
        if (skill.rawActionId <= 0 || member.level < skill.meta.classJobLevel) return;
  ...
  function buildSkillStatusText(member: TeamWatchMemberView, skill: TeamWatchSkillView) {
    ...
    if (!state.resourceReady) suffixes.push(`资源不足(${state.resourceValue ?? 0})`);
    if (state.extraText) suffixes.push(`附加:${state.extraText}`);
  ...
  function getSnapshot() {
    return {
      ...
      watchJobsActionsIDUser: Object.fromEntries(
        Object.entries(watchJobsActionsIDUser.value).map(([k, v]) => [Number(k), [...v]]),
      ),
  ...
  function loadFromStorage() {
    ...
    try {
      ...
      watchJobsActionsIDUser.value = Object.fromEntries(
        Object.entries(next.watchJobsActionsIDUser).map(([k, v]) => [Number(k), [...v]]),
      );
  ...
  function importSettings(encoded: string) {
    ...
    if (!parsed || typeof parsed !== "object") throw new Error("invalid teamwatch payload");

    const entries = Object.entries(parsed);
    const isLegacyV4Payload =
      entries.length > 0 &&
