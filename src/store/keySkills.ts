...
import {
  ...
  resolveActionJobsFromTargets,
  resolveActionMetaByLevel,
...
function normalizeAutoMetaCache(
  raw: Record<string, KeySkillAutoMeta>,
): Record<number, KeySkillAutoMeta> {
  ...
  Object.entries(raw).forEach(([key, value]) => {
    ...
    // 字段缺失则淘汰
    if (!name || !src || !jobs.length) return;
    ...
    output[id] = {
      ...
      isRoleAction,
      minLevel: resolveActionMinLevel(value.minLevel ?? 1, {
        actionId: id,
        isRoleAction,
        fallback: 1,
      }),
...
function normalizeStorageSkills(raw: KeySkillEntry[]): KeySkillEntry[] {
  return raw.flatMap((item) => {
    ...
    if (id <= 0 || key === LEGACY_PRESET_2248_KEY) return [];
...
const useKeySkillStore = defineStore("keySkill", () => {
  ...
  const speed = ref(1);
  const levelSyncTestLevel = useStorage<number>(
    "keySkills-level-sync-test-level",
    GLOBAL_SKILL_MAX_LEVEL,
  );
  const autoMetaCache = useStorage<Record<string, KeySkillAutoMeta>>(
    KEY_SKILLS_AUTO_META_CACHE_STORAGE_KEY,
    {},
  );
  const autoMetaCacheVersion = useStorage<string>(
    KEY_SKILLS_AUTO_META_CACHE_VERSION_STORAGE_KEY,
    "",
  );

  levelSyncTestLevel.value = Math.min(
    GLOBAL_SKILL_MAX_LEVEL,
    Math.max(1, Math.trunc(levelSyncTestLevel.value) || GLOBAL_SKILL_MAX_LEVEL),
  );
  watch(levelSyncTestLevel, (value) => {
    const normalized = Math.min(
      GLOBAL_SKILL_MAX_LEVEL,
      Math.max(1, Math.trunc(value) || GLOBAL_SKILL_MAX_LEVEL),
    );
    if (normalized !== value) levelSyncTestLevel.value = normalized;
  });
  ...
  function buildAutoMetaFromBaked(actionId: number): KeySkillAutoMeta | undefined {
    ...
    const jobs = levelMeta.jobsFromGlobal.length > 0 ? levelMeta.jobsFromGlobal : baked.jobs;
  ...
  async function ensureActionAutoMeta(actionId: number) {
    ...
    if (isApiCached(id)) {
      if (!autoMetaById[id]) {
        ...
        if (cached) autoMetaById[id] = cached;
      }
      return;
    }
    if (refreshedIncompleteAutoMeta.has(id)) return;
    ...
    const nextTask = (async () => {
      ...
      try {
        ...
        const jobs =
          levelMeta.jobsFromGlobal.length > 0
        ...
        const baked = buildAutoMetaFromBaked(id);
        if (baked) autoMetaById[id] = baked;
      } finally {
  ...
  watch(
    ...
    (ids) => {
      uniqueInts(ids.map(Number)).forEach((id) => {
        if (pendingAutoMetaTasks.has(id) || pendingAutoMeta.has(id)) return;
        if (isApiCached(id)) return;
        if (refreshedIncompleteAutoMeta.has(id)) return;
  ...
  const usedSkills = computed(() => {
    ...
    for (const player of currentParty) {
      const level =
        dev.value || demo.value
          ? levelSyncTestLevel.value
          : Math.min(
              GLOBAL_SKILL_MAX_LEVEL,
              Math.max(1, Math.trunc(player.level) || GLOBAL_SKILL_MAX_LEVEL),
            );
      for (const skill of keySkillsData.value.chinese) {
        ...
        if (!resolvedJobs.includes(player.job)) continue;

        const recastInput = skill.recast1000ms ?? globalMeta?.recast1000ms;
        const resolvedRecast =
          (recastInput !== undefined ? parseDynamicValue(recastInput, level) : 0) ||
          meta.recast1000ms;
        const durInput = skill.duration ?? globalMeta?.duration;
        const resolvedDuration =
          (durInput !== undefined ? parseDynamicValue(durInput, level) : 0) || meta.duration;
        const chargesInput = skill.maxCharges ?? globalMeta?.maxCharges;
        const resolvedMaxCharges =
          (chargesInput !== undefined ? parseDynamicValue(chargesInput, level) : 1) || 1;

        const resolvedMinLevel = resolveActionMinLevel(skill.minLevel ?? meta.minLevel, {
          actionId: meta.id,
          isRoleAction: meta.isRoleAction,
          fallback: meta.minLevel,
        });
        if (resolvedMinLevel > level) continue;
    ...
    result.sort((a, b) => {
      ...
      if (aIndex === bIndex) {
        return (
          DEFAULT_JOB_SORT_ORDER.indexOf(a.owner.job) - DEFAULT_JOB_SORT_ORDER.indexOf(b.owner.job)
        );
      }
  ...
  function triggerSkill(skillIds: number[], ownerID: string, speak: boolean) {
    const matchedSkills = usedSkills.value.filter(
    ...
    matchedSkills.forEach((skill) => {
      ...
      if (speak && enableTts.value.chinese) void tts(skill.tts);
    });
  }
  ...
  function resetSkillsToDefault() {
    keySkillsData.value = {
      chinese: defaultSkillEntries.map((skill) =>
        createSkillEntry(skill.id, skill.tts, skill.line),
      ),
      mergedIds: [...defaultSkillIds],
    };
    wipe();
