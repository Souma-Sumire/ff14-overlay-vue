...
class Timeline implements ITimeline {
  constructor(name: string, condition: ITimelineCondition, timeline: string, source: string) {
...
export const useTimelineStore = defineStore("timeline", {
  ...
  actions: {
    ...
    newTimeline(
      ...
      this.allTimelines.push(new Timeline(title, condition, rawTimeline, source));
      this.sortTimelines();
    ...
    sortTimelines() {
      ...
      this.allTimelines.sort((a, b) => {
        ...
        // 3. 职业
        const jobDiff =
          Util.jobToJobEnum(a.condition.jobs[0]!) - Util.jobToJobEnum(b.condition.jobs[0]!);
...
function parseTime(time: string): number {
  const timeFormatType = time.match(/^(?<negative>-)?(?<mm>\d+):(?<ss>\d+(?:\.\d*)?)$/);
  if (timeFormatType) {
...
export async function parseTimeline(rawTimeline: string): Promise<ITimelineLine[]> {
  ...
  const matches = [
    ...rawTimeline.matchAll(/^\s*(?<time>[-:：\d.]+)\s+(?<action>["'“”][^"'“”]*["'“”]).*$/gm),
  ];
  for (let i = 0; i < matches.length; i++) {
    ...
    const ttsSim = / tts(?: |$)/.test(match[0])
      ? Array.from(parseAction(match.groups?.action ?? ""), (item) => item?.groups?.name)
          .filter(Boolean)
          .join(", ")
      : undefined;
    ...
    try {
      ...
      ElMessage.error({
        type: "error",
        message: `“${match[0] ?? ""}”解析失败：${e instanceof Error ? e.message : String(e)}`,
      });
      break;
    }
    ...
    if (cactbotRegexType) {
      try {
        ...
        const regex = Regexes[key as keyof typeof Regexes] as (params: unknown) => RegExp;
