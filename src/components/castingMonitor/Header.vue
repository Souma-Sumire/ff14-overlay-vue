...
watchEffect(() => {
  for (const item of castingMonitorStore.partyData) item.src = getClassjobIconSrc(item.job);
});
...
<template>
  <div
    v-if="
      showHeader && castingMonitorStore.type === 'focus' && castingMonitorStore.partyData.length > 1
    "
    z-100
    flex="~ gap0 wrap"
    class="header-layout"
