...
const damageNumberDisplayUnit = computed<DamageNumberDisplayUnit>(() =>
  props.damageNumberDisplayUnit === "k" ? "k" : "w",
);

const allMechanics = computed(() => props.mechanicOptions ?? []);
const sourceRows = computed(() =>
  props.allRows && props.allRows.length > 0 ? props.allRows : props.rows,
);
...
const aoeMechanics = computed(() =>
  allMechanics.value.filter((mechanic) => {
    const type = mechanicTypeMap.value.get(mechanic.actionId);
    return !!type?.isAOE && !type.isShare;
  }),
);
const shareMechanics = computed(() =>
  allMechanics.value.filter((m) => mechanicTypeMap.value.get(m.actionId)?.isShare),
);
const tbMechanics = computed(() =>
  allMechanics.value.filter((m) => mechanicTypeMap.value.get(m.actionId)?.isTB),
);
const normalMechanics = computed(() =>
  allMechanics.value.filter((mechanic) => {
    const type = mechanicTypeMap.value.get(mechanic.actionId);
...
function getGroupSelectionState(items: Array<{ actionId: string }>) {
  if (items.length === 0) return { selected: false, indeterminate: false };
...
function toggleMechanicGroup(items: Array<{ actionId: string }>, val: boolean) {
  ...
  if (val) {
    for (const id of targetIds) {
      if (!currentFilterSet.value.has(id)) next.push(id);
    }
  } else {
...
const mechanicEditContextMenu = ref<{
  top: number;
  left: number;
  anchorTop: number;
  anchorLeft: number;
...
const mechanicTargetContextMenu = ref<{
  top: number;
  left: number;
  anchorTop: number;
  anchorLeft: number;
...
function getContextOverlayEstimatedSize(id: ContextOverlayId) {
  if (id === "mechanic-ctx") return { width: 300, height: 280 };
  if (id === "mechanic-edit-ctx") return { width: 320, height: 220 };
  if (id === "mechanic-target-ctx") return { width: 320, height: 180 };
  if (id === "column-ctx") return { width: 220, height: 278 };
  if (id === "skill-ctx") return { width: 240, height: 260 };
  return { width: 180, height: 140 };
}

function clampMenuPosition(left: number, top: number, width = 0, height = 0) {
  const viewportWidth = window.innerWidth || 0;
  const viewportHeight = window.innerHeight || 0;
  const maxLeft = Math.max(
    CONTEXT_MENU_VIEWPORT_PADDING,
    viewportWidth - width - CONTEXT_MENU_VIEWPORT_PADDING,
  );
  const maxTop = Math.max(
    CONTEXT_MENU_VIEWPORT_PADDING,
    viewportHeight - height - CONTEXT_MENU_VIEWPORT_PADDING,
  );
  return {
...
function getContextMenuElement(overlayId: ContextOverlayId) {
  if (overlayId === "mechanic-ctx") return mechanicContextMenuRef.value;
  if (overlayId === "mechanic-edit-ctx") return mechanicEditContextMenuRef.value;
  if (overlayId === "mechanic-target-ctx") return mechanicTargetContextMenuRef.value;
  if (overlayId === "column-ctx") return columnContextMenuRef.value;
...
const selectedSkillAutoArrangeUnsupportedReason = computed(() => {
  ...
  if (!skill) return "当前技能不可用";
  if (!skill.mitigationScope) return "该技能不支持自动排轴";
  return "";
});

const isSelectedSkillAutoArrangeSupported = computed(
  () => !selectedSkillAutoArrangeUnsupportedReason.value,
);
...
function updateSkillDetailVisible(columnKey: string, skillId: number, visible: boolean) {
  ...
  if (activeSkillDetailKey.value === key) activeSkillDetailKey.value = null;
}
...
watch(activeOverlay, (val) => {
  if (val) hideSkillDetail();
  if (val) {
    cellPopover.visible = false;
    if (isContextOverlay(val)) void ensureContextMenuPositionInViewport(val);
  } else {
...
function buildTimelineText(type: "skill" | "column") {
  ...
  const filteredActions = actions.filter((a) => {
    if (targetSkillId !== null) return a.skillId === targetSkillId;
...
function isTimestampClose(
  ...
  tolerance = TIMESTAMP_EPSILON,
) {
  if (!Number.isFinite(left) || !Number.isFinite(right)) return false;
  return Math.abs((left || 0) - (right || 0)) <= tolerance;
}

function matchesCellAction(
...
function findActionIndexByCell(
  actions: PlayerActionRecord[],
  rowKey: string,
  columnKey: string,
  skillId: number,
...
function findActionByUseTimestamp(
  ...
  if (!Number.isFinite(useTimestamp)) return undefined;
  return actions.find((a) => {
    return (
      a.columnKey === columnKey &&
      a.skillId === skillId &&
...
function findLatestCoveringActionIndex(
  ...
  return (
    actions
      ...
      .filter(
        ({ a }) =>
          rowTimestamp >= a.timestamp && (duration <= 0 || rowTimestamp < a.timestamp + duration),
      )
      .sort((x, y) => y.a.timestamp - x.a.timestamp)[0]?.index ?? -1
  );
}
...
function validateActionPlacement(
  ...
  if (recastWindow > 0 && !isChargeBased) {
    for (const action of props.playerActions) {
      if (ignoreAction && action.id && action.id === ignoreAction.id) continue;
    ...
    const allUsages = props.playerActions
      .filter(
        (a) =>
          a.columnKey === columnKey &&
          a.skillId === skillId &&
          (!ignoreAction || a.id !== ignoreAction.id),
      )
      .map((a) => a.timestamp);
...
function shouldShowCellUseIcon(
  row: MitigationRow,
  rowIndex: number,
  col: ColumnDef,
  skillId: number,
...
function shouldMuteCellUseIcon(row: MitigationRow, col: ColumnDef, skillId: number) {
  const info = getCellSim(row, col, skillId);
  if (!info) return false;
  if (info.showDot) return !!info.showDotMuted;
  return false;
}
...
function toggleCell(row: MitigationRow, rowIndex: number, col: ColumnDef, skillId: number) {
  ...
  if (isChecked) {
    ...
    const fallbackIndex = findLatestCoveringActionIndex(
      newActions,
      columnKey,
      skillId,
      row.timestamp,
...
function findEditOffsetBoundary(
  ...
  for (let i = 0; i < EDIT_OFFSET_BOUNDARY_SCAN_LIMIT; i++) {
    ...
    if (!canUpdateEditOffsetWithContext(ctx, next)) return current;
    current = next;
  }
...
const editOffsetLowerLabel = computed(() => {
  if (!editOffsetBounds.value) return "--";
...
const editOffsetModel = computed<number>({
  ...
  set: (value) => {
    ...
    if (!canUpdateEditOffset(nextOffset)) return;
...
function getCellClasses(row: MitigationRow, rowIndex: number, col: ColumnDef, skillId: number) {
  ...
  const sameUsePrev =
    cellSim.useTimestamp !== undefined &&
    cellSim.useTimestamp === getCellSim(prevRow || row, col, skillId)?.useTimestamp;
  const sameUseNext =
    cellSim.useTimestamp !== undefined &&
    cellSim.useTimestamp === getCellSim(nextRow || row, col, skillId)?.useTimestamp;

  const connPrev =
    (isActiveStatus(status) && isActiveStatus(prevStatus)) ||
  ...
  const connNext =
    (isActiveStatus(status) && isActiveStatus(nextStatus)) ||
...
function getStatusLabel(status: string | undefined) {
  if (status === "active-start") return "释放瞬间";
  if (status === "active") return "生效中";
  if (status === "cooldown") return "冷却中";
  if (status === "conflict") return "预冷却";
  return "准备就绪";
}
...
function getStatusLabelForCell(
  ...
  if (status === "active-start") {
    ...
    if (useTimestamp !== undefined) {
      ...
      if (elapsed >= 0 && elapsed < activeDuration - 0.001) return "生效中";
      return "冷却中";
    }
    return "释放瞬间";
  }
...
function getRoleClassByJobEnum(jobEnum: number) {
  ...
  if (Util.isTankJob(job)) return "role-tank";
  if (Util.isHealerJob(job)) return "role-healer";
  if (Util.isDpsJob(job)) return "role-dps";
  return "";
}
...
const targetRoleClassByName = computed(() => {
  ...
  props.columns.forEach((col) => {
    ...
    if (roleClass) map.set(displayName, roleClass);
  });
  return map;
});
...
function resolveMechanicTargetColumnKeys(row: MitigationRow) {
  const rowTargetIds = new Set(
    (row.targetIds || []).map((item) => String(item || "").trim()).filter(Boolean),
  );
  const rowTargets = new Set(
    (row.targets || []).map((item) => String(item || "").trim()).filter(Boolean),
  );
  const keys: string[] = [];
  for (const col of props.columns) {
    ...
    if (nameCandidates.size > 0 && [...nameCandidates].some((name) => rowTargets.has(name))) {
      keys.push(col.key);
      continue;
    }
    if (rowTargets.has(getMitigationColumnDisplayName(col))) keys.push(col.key);
  }
  return keys;
}
...
function getSkillScopeLabel(scope: MitigationSkill["mitigationScope"]) {
  if (scope === "party") return "团队";
  if (scope === "self") return "自身";
  return "未定义";
}
...
function openMechanicContextMenu(
  ...
  if (overlayId === "mechanic-ctx") mechanicContextMenu.value = nextState;
  else if (overlayId === "mechanic-edit-ctx") mechanicEditContextMenu.value = nextState;
  else mechanicTargetContextMenu.value = nextState;
    ...
    .then((text) => {
      ...
      try {
        ...
        if (importedCol.j !== targetCol.jobEnum) {
          const importedJob =
            Util.jobToFullName(Util.jobEnumToJob(importedCol.j || 0)).cn || "未知职业";
        ...
        function applyImport(colData: ImportedColumnPayload, skillsMap: Record<string, number[]>) {
          ...
          const newCols = props.columns.map((c) =>
            c.key === targetCol.key ? { ...c, hiddenSkillIds: colData.h || [] } : c,
          );
...
const cellPopoverInfo = computed(() => {
  if (!cellPopover.row || !cellPopover.col || !cellPopover.skill) return null;
...
function getSkillMultiplier(skillId: number, damageType: MitigationRow["damageType"]) {
  if (damageType === "dot" || damageType === "heal") return 1;
  ...
  if (damageType === "physics") return multiplier.physics;
  if (damageType === "magic") return multiplier.magic;
  if (damageType === "darkness") return multiplier.darkness;
  return 1;
}
...
function getAutoArrangeCoverageScope(skillId: number): AutoArrangeCoverageScope | null {
  const skill = skillById.value.get(skillId);
  if (skill?.mitigationScope === "party") return "party";
  if (skill?.mitigationScope === "self") return "self";
  return null;
}

function isExtremeFailureDamageRow(row: MitigationRow) {
  const rawDamage = Number(row.rawDamage || 0);
  if (Number.isFinite(rawDamage) && rawDamage > EXTREME_DAMAGE_THRESHOLD) return true;
  return !!row.damageDetails?.some((detail) => {
    const estimatedRaw = Number(detail.estimatedRaw || 0);
    if (Number.isFinite(estimatedRaw) && estimatedRaw > EXTREME_DAMAGE_THRESHOLD) return true;
...
function isRowTargetingAllPartyStrictForDamage(row: MitigationRow) {
  ...
  if (!partyColumns.length) return false;

  const rowTargetIds = new Set(
    (row.targetIds || []).map((item) => normalizeTargetId(item)).filter(Boolean),
  );
  const detailTargetIds = new Set(
    (row.damageDetails || []).map((detail) => normalizeTargetId(detail.targetId)).filter(Boolean),
  );
  const allTargetIds = new Set<string>([...rowTargetIds, ...detailTargetIds]);
  const rowTargets = new Set(
    (row.targets || []).map((item) => String(item || "").trim()).filter(Boolean),
  );
  if (rowTargets.has(TARGET_ALL_LABEL)) return true;

  let matched = 0;
  for (const col of partyColumns) {
    const colTargetId = normalizeTargetId(col.targetId);
    if (
      (colTargetId && allTargetIds.has(colTargetId)) ||
      allTargetIds.has(normalizeTargetId(col.key))
    ) {
      matched += 1;
      continue;
    }
  ...
  if (matched >= partyColumns.length) return true;
...
function getPersonalDamageForColumn(row: MitigationRow, col: ColumnDef) {
  ...
  if (details && details.length > 0) {
    ...
    if (targetId) {
      ...
      if (byId > 0) matchedDamage = Math.max(matchedDamage, byId);
    }
    ...
    if (nameCandidates.size > 0) {
      const byName = details
        ...
        .reduce((sum, detail) => sum + getDetailBaseDamage(detail), 0);
      if (byName > 0) matchedDamage = Math.max(matchedDamage, byName);
    }
    if (targetsAll || rowHitsAllParty) return Math.max(matchedDamage, fallback);
    if (matchedDamage > 0) return matchedDamage;
    return 0;
  }
  ...
  if (
    targetsAll ||
    (nameCandidates.size > 0 && targetNames.some((target) => nameCandidates.has(target)))
  )
    return fallback;
  if (rowHitsAllParty) return fallback;
...
function getRowReductionRate(row: MitigationRow) {
  ...
  for (const [cellKey, cell] of Object.entries(cells)) {
    if (cell.status !== "active" && cell.status !== "active-start") continue;
    const skillIdMatch = cellKey.match(/_(\d+)$/);
    if (!skillIdMatch) continue;
    ...
    const normalizedTimestamp = Number.isFinite(useTimestamp)
      ? useTimestamp
...
function getAutoArrangeStrategyLabel(strategy: AutoArrangeStrategy) {
  if (strategy === "tb-priority") return "死刑优先";
  if (strategy === "peak-smoothing") return "大技能优先";
  return "HPS优先";
}

function getDefaultAutoArrangeStrategy(col: ColumnDef, skillId: number): AutoArrangeStrategy {
  const coverage = getAutoArrangeCoverageScope(skillId);
  if (!coverage) return "max-total";
  if (coverage === "self" && col.role === "tank") return "tb-priority";
  return "max-total";
}
...
async function chooseAutoArrangeBinaryOption(
  ...
  activeOverlay.value = null;
  try {
    await ElMessageBox.confirm(title, "自动排轴策略", {
...
async function chooseAutoArrangeStrategyForSkill(
  ...
  if (coverage !== "party") return getDefaultAutoArrangeStrategy(col, skillId);
...
async function chooseExclusiveGroupAutoArrangeScope(
  skillName: string,
): Promise<AutoArrangeExclusiveScope | null> {
  const selected = await chooseAutoArrangeBinaryOption(
...
function findEarliestValidAutoArrangeReleaseTimestamp(params: {
  ...
  if (minTimestamp !== undefined) earliest = Math.max(earliest, minTimestamp);
  if (earliest > latest + AUTO_PLAN_EPSILON) return null;
  const step = 0.5;
...
function optimizeAutoArrangedActionOffsets(params: {
  ...
  const { rows, actions, buildContext, targetActionIds, anchorRowKeyByActionId, resolveStrategy } =
    params;
  if (targetActionIds.size === 0 || rows.length === 0 || actions.length === 0) return actions;
  ...
  orderedIds.forEach((actionId) => {
    ...
    const resolvedRowIndex =
      anchorRowIndex ?? resolveActionRowIndex(rows, buildContext.rowIndexByKey, action);
    ...
    const rowCache: AutoArrangeRowComputationCache = {
      ...
      personalDamageByRow:
        coverage === "self" ? buildPersonalDamageCacheForColumn(rows, col) : undefined,
    ...
    const optimizedTimestamp = findEarliestValidAutoArrangeReleaseTimestamp({
      ...
      isValid: (candidateTimestamp) => {
        if (candidateTimestamp > latestTimestamp + AUTO_PLAN_EPSILON) return false;
        if (
          !validateActionPlacementAgainstActions(
            withoutCurrent,
            col,
            action.skillId,
            candidateTimestamp,
        ...
        return (
          candidateWeight.score + AUTO_PLAN_EPSILON >= baselineWeight.score &&
...
function getRowPriorityWeight(row: MitigationRow, strategy: AutoArrangeStrategy) {
  if (strategy !== "tb-priority") return 1;
  if (row.isTB) return 2.5;
  if (isAoeLike(row)) return 1.25;
  return 1;
}
...
function resolveActionRowIndex(
  rows: MitigationRow[],
  rowIndexByKey: Map<string, number>,
  action: PlayerActionRecord,
) {
  const byTimestamp = findFirstRowAtOrAfter(rows, action.timestamp);
  if (!action.rowKey) return byTimestamp;

  const byKey = rowIndexByKey.get(action.rowKey);
  if (byKey === undefined) return byTimestamp;
  if (byTimestamp === -1) return byKey;

  const keyRow = rows[byKey];
  const tsRow = rows[byTimestamp];
  const keyTimeMatches =
    !!keyRow && Math.abs(keyRow.timestamp - action.timestamp) <= AUTO_PLAN_EPSILON;
  const tsTimeMatches =
    !!tsRow && Math.abs(tsRow.timestamp - action.timestamp) <= AUTO_PLAN_EPSILON;
...
function buildScheduledActions(
  ...
  const rowIndexByKey =
    context?.rowIndexByKey || new Map(rows.map((row, index) => [row.key, index]));
...
function isScheduledActionActiveAtRow(
  action: AutoScheduledAction,
  rowIndex: number,
  rowTimestamp: number,
) {
  if (action.rowIndex === rowIndex) return true;
  if (action.duration <= AUTO_PLAN_EPSILON) return false;
  const elapsed = rowTimestamp - action.timestamp;
  return elapsed >= 0 && elapsed < action.duration;
}

function computeBaselineMultipliers(
  rows: MitigationRow[],
  ...
  return rows.map((row, rowIndex) => {
    ...
    scheduledActions.forEach((action) => {
      if (!isScheduledActionActiveAtRow(action, rowIndex, row.timestamp)) return;
      const coverage = getAutoArrangeCoverageScope(action.skillId);
      if (coverage === "self") {
        const ownerCol = columnByKey.get(action.columnKey);
        if (!ownerCol || getPersonalDamageForColumn(row, ownerCol) <= AUTO_PLAN_EPSILON) return;
      }
      const actionMultiplier = getSkillMultiplier(action.skillId, row.damageType);
      if (actionMultiplier >= 1) return;
...
function isSkillAlreadyActiveAtRow(
  ...
  return scheduledSkillActions.some((action) =>
    isScheduledActionActiveAtRow(action, rowIndex, rowTimestamp),
  );
}
...
function getExclusiveGroupActions(
  ...
  if (!exclusiveGroup) return EMPTY_SCHEDULED_ACTIONS;
...
function hasExclusiveGroupOverlap(
  ...
  if (exclusiveGroupActions.length === 0) return false;
  ...
  return exclusiveGroupActions.some((action) => {
    ...
    return (
      action.timestamp < endTimestamp - AUTO_PLAN_EPSILON &&
...
function computeAutoUseWeightForReleaseTimestamp(
  ...
  const coverage = getAutoArrangeCoverageScope(skillId);
  if (!coverage) return { gain: 0, score: 0 };
  ...
  rows.forEach((row, rowIndex) => {
    const isWithinDuration =
      duration > AUTO_PLAN_EPSILON &&
      row.timestamp >= startTimestamp &&
      row.timestamp < endTimestamp;
    if (!isWithinDuration) return;

    const isExtremeFailure =
      cache?.extremeFailureMask?.[rowIndex] ?? isExtremeFailureDamageRow(row);
    if (isExtremeFailure) return;
    const personalDamage = coverage === "self" ? (cache?.personalDamageByRow?.[rowIndex] ?? 0) : 0;

    if (!hasDamageTakenMultiplier) {
      if (coverage === "self" && personalDamage <= AUTO_PLAN_EPSILON) return;
      const gain = 1;
      totalGain += gain;
      let score = gain;
      if (strategy === "tb-priority") score *= getRowPriorityWeight(row, strategy);
      totalScore += score;
      return;
    }

    const skillMultiplier =
      cache?.skillMultiplierByRow?.[rowIndex] ?? getSkillMultiplier(skillId, row.damageType);
    if (skillMultiplier >= 1) return;

    const baseDamage =
      coverage === "self" ? Math.max(0, personalDamage) : Math.max(0, Number(row.rawDamage || 0));
...
function validateActionPlacementAgainstActions(
  ...
  const duration = Math.max(0, Number(skill?.duration || 0));
  const groupActions =
    exclusiveGroupActions || getExclusiveGroupActions(scheduledActions, exclusiveGroup);

  if (skill?.isTargetMitigation && newTimestamp < 0) return false;

  if (hasExclusiveGroupOverlap(groupActions, newTimestamp, duration)) return false;
  ...
  if (isChargeBased && skill) {
    ...
    for (const action of actions) {
      ...
      if (Math.abs(ts - newTimestamp) <= AUTO_PLAN_EPSILON) return false;
      allUsages.push(ts);
    }
    allUsages.push(newTimestamp);
    allUsages.sort((a, b) => a - b);
...
function getExclusiveGroupAutoArrangeUnits(
  ...
  props.columns.forEach((column) => {
    ...
    getVisibleSkills(column).forEach((visibleSkill) => {
      if (getExclusiveGroupBySkillId(visibleSkill.id) !== exclusiveGroup) return;
      if (!visibleSkill.mitigationScope) return;
  ...
  if (!fallbackSkill.mitigationScope) return units;
...
function getLatestUsageByUnit(
  ...
  actions.forEach((action) => {
    const key = `${action.columnKey}_${action.skillId}`;
    if (!unitKeySet.has(key)) return;
    const previous = latest.get(key);
    if (previous === undefined || action.timestamp > previous) latest.set(key, action.timestamp);
  });
  return latest;
}
...
function getExclusiveGroupUnitPriority(
  ...
  if (headerOrder !== undefined) return headerOrder;
...
function compareExclusiveGroupCandidate(
  ...
  if (!best) return true;
  if (current.score > best.score + AUTO_PLAN_EPSILON) return true;
  if (Math.abs(current.score - best.score) <= AUTO_PLAN_EPSILON) {
    if (current.rhythmPenalty < best.rhythmPenalty - AUTO_PLAN_EPSILON) return true;
    if (Math.abs(current.rhythmPenalty - best.rhythmPenalty) <= AUTO_PLAN_EPSILON) {
      if (current.gain > best.gain + AUTO_PLAN_EPSILON) return true;
      if (Math.abs(current.gain - best.gain) <= AUTO_PLAN_EPSILON) {
        if (current.timestamp < best.timestamp - AUTO_PLAN_EPSILON) return true;
        if (Math.abs(current.timestamp - best.timestamp) <= AUTO_PLAN_EPSILON) {
          if (current.unitPriority < best.unitPriority) return true;
          if (current.unitPriority === best.unitPriority) return current.unitKey < best.unitKey;
        }
      }
    }
  }
...
function runAutoArrangeForExclusiveGroup(params: {
  ...
  units.forEach((unit) => {
    ...
    rowCacheByUnit.set(unit.key, {
      extremeFailureMask,
      personalDamageByRow:
        coverage === "self" ? buildPersonalDamageCacheForColumn(rows, unit.col) : undefined,
      skillMultiplierByRow: buildSkillMultiplierCache(rows, unit.skillId),
    });
  });
  const baseActions =
    mode === "overwrite"
      ? props.playerActions.filter(
          (action) => !unitKeySet.has(`${action.columnKey}_${action.skillId}`),
        )
      : [...props.playerActions];
  ...
  const selectBestCandidate = (
    ...
    for (const unit of candidateUnits) {
      ...
      const effectiveDuration = getAutoArrangeEffectiveDuration(duration);
      const recastWindow = Math.max(
        AUTO_PLAN_EPSILON,
        getAutoArrangeRecastWindow(Number(unit.skill.recast || 0)),
      );
      ...
      for (let rowIndex = 0; rowIndex < rows.length; rowIndex++) {
        ...
        if (isSkillAlreadyActiveAtRow(scheduledSkillActions, rowIndex, row.timestamp)) continue;
        if (
          !validateActionPlacementAgainstActions(
            currentActions,
            unit.col,
        ...
        if (result.score <= AUTO_PLAN_EPSILON) continue;

        const latestUsage = latestUsageByUnit.get(unit.key);
        const rhythmPenalty =
          latestUsage === undefined ? 0 : Math.abs(releaseTimestamp - (latestUsage + recastWindow));
  ...
  while (addedCount < maxIterations) {
    ...
    const minTimestamp = firstRoundPreferredOnly
      ? undefined
      : (firstPreferredTimestamp ?? undefined);
  ...
  const optimizedActions = optimizeAutoArrangedActionOffsets({
    ...
    resolveStrategy: (col, skillId) => {
      const coverage = getAutoArrangeCoverageScope(skillId);
      if (coverage === "party") return partyStrategy;
...
async function runAutoArrangeForSkill() {
  ...
  const exclusiveGroup = getExclusiveGroupBySkillId(skillId);
  const exclusiveUnits = exclusiveGroup
    ? getExclusiveGroupAutoArrangeUnits(exclusiveGroup, col, skill)
    : [];
  ...
  if (exclusiveUnits.length > 1) {
    ...
    if (exclusiveScope === "group") {
      const strategyForGroup =
        selectedStrategy === "peak-smoothing" ? "peak-smoothing" : "max-total";
      ...
      ElMessage.success(
        `已自动排轴互斥组（${result.addedCount} 次，预计减伤 ${Math.round(result.totalGain)}，策略：${getAutoArrangeStrategyLabel(strategyForGroup)}）`,
      );
      return;
    }
  }

  const baseActions = props.playerActions.filter(
    (a) => !(a.columnKey === columnKey && a.skillId === skillId),
  );
  ...
  rows.forEach((row, rowIndex) => {
    const releaseTimestamp = getAutoArrangeReleaseTimestamp(row.timestamp);
    if (skill.isTargetMitigation && releaseTimestamp < 0) return;
    if (
      !validateActionPlacementAgainstActions(
        baseActions,
        col,
        skillId,
        releaseTimestamp,
        baseScheduledActions,
        baseExclusiveGroupActions,
      )
    )
      return;
    const result = computeAutoUseWeightForReleaseTimestamp(
    ...
    if (!Number.isFinite(result.score) || result.score <= AUTO_PLAN_EPSILON) return;
    candidates.push({
      rowIndex,
      timestamp: releaseTimestamp,
      score: result.score,
  ...
  function normalizeRechargeAts(rechargeAts: number[], currentTime: number) {
    return Array.from(rechargeAts, (rt) =>
      rt <= currentTime + AUTO_PLAN_EPSILON ? Number.NEGATIVE_INFINITY : rt,
    ).sort((a, b) => a - b);
  }
    ...
    if (Math.abs(current.score - best.score) <= AUTO_PLAN_EPSILON) {
      ...
      if (Math.abs(current.gain - best.gain) <= AUTO_PLAN_EPSILON) {
        ...
        for (let i = 0; i < maxLength; i++) {
          ...
          if (bestIndex === undefined) return true;
          ...
          if (currentTs < bestTs - AUTO_PLAN_EPSILON) return true;
          if (currentTs > bestTs + AUTO_PLAN_EPSILON) return false;
        }
      }
    }
    return false;
  ...
  function solveAutoArrangeDP(
    ...
    if (index >= candidates.length) return { score: 0, gain: 0, picks: [] };

    const candidate = candidates[index];
    if (!candidate) return { score: 0, gain: 0, picks: [] };

    const normalizedRechargeAts = isChargeBased
      ? normalizeRechargeAts(rechargeAts, candidate.timestamp)
      : rechargeAts;
    const rechargeKey = isChargeBased ? normalizedRechargeAts.map(formatPlanTimeKey).join(",") : "";
    ...
    if (canTake && isChargeBased) {
      const availableIndex = normalizedRechargeAts.findIndex(
        (rt) => !Number.isFinite(rt) || rt <= candidate.timestamp + AUTO_PLAN_EPSILON,
      );
      if (availableIndex === -1) {
        canTake = false;
      } else {
    ...
    if (canTake) {
      ...
      if (isPlanBetter(currentPlan, bestPlan)) bestPlan = currentPlan;
    }
  ...
  const initialRechargeAts: number[] = isChargeBased
  ...
  ElMessage.success(
    `已自动排轴 ${skill.name}（${appliedCount} 次，预计减伤 ${totalGain}，策略：${strategyLabel}）`,
  );
}
...
async function runAutoArrangeForSkillFillGaps() {
  ...
  if (exclusiveUnits.length > 1) {
    ...
    if (selectedScope === "group") {
      ...
      emit("update:playerActions", result.actions);
      ElMessage.success(
        `已补全互斥组 ${result.addedCount} 次（预计减伤 ${Math.round(result.totalGain)}，策略：${getAutoArrangeStrategyLabel(strategyForGroup)}）`,
      );
      return;
    }
  }
  ...
  while (true) {
    ...
    const scheduledSkillActions =
      scheduledByUnitKey.get(`${columnKey}_${skillId}`) || EMPTY_SCHEDULED_ACTIONS;
    const scheduledExclusiveGroupActions = getExclusiveGroupActions(
      scheduledActions,
      exclusiveGroup,
    );
    ...
    rows.forEach((row, rowIndex) => {
      const releaseTimestamp = getAutoArrangeReleaseTimestamp(row.timestamp);
      if (isSkillAlreadyActiveAtRow(scheduledSkillActions, rowIndex, row.timestamp)) return;
      if (
        !validateActionPlacementAgainstActions(
          currentActions,
          col,
      ...
      if (
        result.score > bestScore + AUTO_PLAN_EPSILON ||
        (Math.abs(result.score - bestScore) <= AUTO_PLAN_EPSILON &&
          result.gain > bestGain + AUTO_PLAN_EPSILON)
      ) {
    ...
    if (bestRowIndex < 0 || bestScore <= AUTO_PLAN_EPSILON) break;
    ...
    addedCount += 1;
    totalGain += bestGain;
    if (addedCount > rows.length) break;
  }
  ...
  const strategyLabel = getAutoArrangeStrategyLabel(selectedStrategy);
  ElMessage.success(
    `已补全 ${skill.name} ${addedCount} 次（预计减伤 ${Math.round(totalGain)}，策略：${strategyLabel}）`,
  );
}
...
function clearSkillActions() {
  ...
  const columnKey = col.key;
  const skillId = skill.id;
  const nextActions = props.playerActions.filter(
    (a) => !(a.columnKey === columnKey && a.skillId === skillId),
  );
...
function triggerSkillAutoArrange(mode: "overwrite" | "fill") {
  if (!isSelectedSkillAutoArrangeSupported.value) return;
  if (mode === "overwrite") {
...
function applyMechanicTargetsToCurrent() {
  ...
  const currentTargets = (row.targets || [])
    ...
    .filter(Boolean);
  if (
    isSameTargetList(currentTargets, nextTargets) &&
    Number(row.targetCount || 0) === nextTargetCount &&
...
function toggleMechanicTargetColumn(columnKey: string) {
  ...
  if (!normalized || normalized === TARGET_ALL_VALUE) return;
  mechanicEditor.targetAll = false;
  const selected = new Set(
    mechanicEditor.targetColumnKeys.map((key) => String(key || "").trim()).filter(Boolean),
  );
...
function toggleMechanicTargetAll() {
  ...
  if (mechanicEditor.targetAll) mechanicEditor.targetColumnKeys = [];
  applyMechanicTargetsToCurrent();
}

function handleMechanicTypeChange(value: unknown) {
  if (!isMechanicEditorType(value)) return;
  const row = mechanicEditor.row;
  if (!row) return;

  updateRows((r) => {
    if (r.actionId !== row.actionId) return r;
    return {
      ...r,
      isAOE: value === "aoe",
      isShare: value === "share",
...
function applyMechanicEditor(options?: { closeMenu?: boolean; showMessage?: boolean }) {
  ...
  const hasChange = sourceRows.value.some(
    (r) => r.actionId === row.actionId && Number(r.rawDamage || 0) !== damage,
  );
  if (!hasChange) return;
  ...
  if (showMessage) ElMessage.success("机制数值修改成功");
}
...
async function deleteCurrentMechanicRow() {
  ...
  try {
    await ElMessageBox.confirm(`将永久删除该机制行：\n${actionSummary}`, "永久删除本行", {
      type: "warning",
  ...
  if (nextActions.length !== props.playerActions.length) emit("update:playerActions", nextActions);
...
async function deleteMechanicRowsByActionId() {
  ...
  try {
    await ElMessageBox.confirm(confirmText, "按技能ID批量删除", {
      type: "warning",
      confirmButtonText: "删除",
      cancelButtonText: "取消",
    });
  ...
  const nextActions = props.playerActions.filter(
    (action) => !action.rowKey || !targetRowKeys.has(action.rowKey),
  );
  if (nextActions.length !== props.playerActions.length) emit("update:playerActions", nextActions);

  if (props.filterMechanics?.includes(actionId)) {
    emit(
      "update:filterMechanics",
      props.filterMechanics.filter((id) => id !== actionId),
    );
  }
...
function getCastStartDisplay(row: MitigationRow) {
  if (row.castTime !== undefined && row.castTime !== null && row.castTime !== "") {
    ...
    if (Number.isFinite(castSeconds)) return formatTime(row.timestamp - castSeconds);
  }
...
function formatDamageNumber(value: unknown) {
  ...
  if (!Number.isFinite(unitScale) || unitScale <= 0) return `${Math.round(n)}`;
...
function getNormalizedHiddenSkillSet(col: ColumnDef) {
  ...
  if (col.skills.length === 0) return hidden;
...
const visibleSkillsByColumn = computed(() => {
  ...
  props.columns.forEach((col) => {
    ...
    map.set(
      col.key,
      col.skills.filter((skill) => !hidden.has(skill.id)),
    );
  });
  return map;
});
...
function toggleSkillVisibility(col: ColumnDef, skillId: number) {
  ...
  const newCols = props.columns.map((c) => {
    ...
    if (hiddenSet.has(skillId)) {
      ...
      const visibleCount = c.skills.reduce(
        (count, skill) => count + (hiddenSet.has(skill.id) ? 0 : 1),
        0,
      );
      if (visibleCount <= 1) {
        blocked = true;
...
function handlePartyColumnJobChange(col: ColumnDef, value: unknown) {
  ...
  if (Number.isNaN(jobEnum) || col.jobEnum === jobEnum) return;
...
function handleColumnNameClick(col: ColumnDef) {
  if (isColumnDragging.value || isJustFinishedColumnDrag.value) return;
  ...
  >
    ...
    <transition name="menu-fade">
      ...
      >
        ...
        <div
          v-if="mechanicContextMenu.row"
          class="mechanic-inline-editor mechanic-inline-editor-compact"
        >
          <div class="mechanic-inline-row">
            ...
            >
              <el-radio-button value="aoe"> AOE </el-radio-button>
              <el-radio-button value="share"> 分摊 </el-radio-button>
              <el-radio-button value="tb"> 死刑 </el-radio-button>
    ...
    <transition name="menu-fade">
      <div
        ...
        class="custom-context-menu"
        :style="{
          top: `${mechanicEditContextMenu.top}px`,
          left: `${mechanicEditContextMenu.left}px`,
        }"
        @click.stop
        @contextmenu.prevent
      >
    ...
    <transition name="menu-fade">
      <div
        ...
        class="custom-context-menu"
        :style="{
          top: `${mechanicTargetContextMenu.top}px`,
          left: `${mechanicTargetContextMenu.left}px`,
        }"
        @click.stop
        @contextmenu.prevent
      >
        ...
        <div v-if="mechanicTargetContextMenu.row" class="mechanic-inline-editor">
          <div class="target-column-list">
            ...
            <button
              ...
              :class="[
                option.roleClass,
                { 'is-active': mechanicEditor.targetColumnKeys.includes(option.value) },
              ]"
    ...
    >
      <div v-if="cellPopoverInfo" class="popover-list cell-popover">
        ...
        <div class="popover-item">
          状态
          {{
            getStatusLabelForCell(
              cellPopoverInfo.sim.status,
              cellPopoverInfo.sim.useTimestamp,
              cellPopoverInfo.row.timestamp,
              cellPopoverInfo.skill.duration,
            )
          }}
        </div>
        ...
        <div
          v-if="
            cellPopoverInfo.sim.useTimestamp !== undefined ||
            cellPopoverInfo.sim.status === 'conflict' ||
            cellPopoverInfo.sim.status === 'cooldown'
          "
          class="popover-item"
        >
        ...
        <div
          v-if="cellPopoverInfo.sim.status === 'conflict' && cellPopoverInfo.sim.conflictTime"
...
.fixed-w-time {
  width: var(--mg-fixed-w-time);
  justify-content: center;
  color: #888;
  user-select: text;
}
.fixed-w-cast-start {
  width: var(--mg-fixed-w-cast-start);
  justify-content: center;
  color: #888;
  user-select: text;
}
.fixed-w-name {
  min-width: var(--mg-fixed-w-name);
  width: fit-content;
  flex: 0 1 auto;
  padding-left: 4px;
  user-select: text;
...
.fixed-w-dmg {
  width: var(--mg-fixed-w-dmg);
  justify-content: flex-end;
  font-size: 11px;
  padding-right: 4px;
}
.fixed-w-shield {
  width: var(--mg-fixed-w-shield);
  justify-content: center;
}
.fixed-w-reduction {
  width: var(--mg-fixed-w-reduction);
  justify-content: center;
}
...
.skill-icon-wrap {
  width: var(--mg-skill-icon-size);
  height: var(--mg-skill-icon-size);
...
.cell-check {
  width: var(--mg-skill-cell-size);
  height: calc(var(--mg-skill-cell-size) - 1px);
...
:deep(
  .mg-auto-arrange-choice-dialog .el-message-box__btns .el-button.mg-auto-arrange-choice-btn:hover
) {
