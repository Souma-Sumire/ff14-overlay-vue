...
import { factory } from "@/resources/timelineSpecialRules";
...
async function queryFFlogsReportFights(url: string) {
  ...
  try {
    ...
    if (
      fight.difficulty === 101 &&
      bossPhase[fight.zoneID] &&
      bossPhase[fight.zoneID]?.fflogsBoss
    ) {
      fflogsQueryConfig.phase =
        bossPhase[fight.zoneID]!.fflogsBoss! === fight.boss ? "final" : "door";
    }
...
async function handleFFlogsQueryResultFriendliesList(player: Friendlies) {
  ...
  try {
    ...
    fflogsQueryConfig.abilityFilterCandidate = fflogsQueryConfig.abilityFilterEvents
      .reduce((total, event) => {
        if (event.sourceIsFriendly && !total.some((v) => v.actionId === event.actionId)) {
          total.push(event);
        }
        return total;
      }, [] as FFlogsStance)
    ...
    fflogsQueryConfig.abilityFilterSelected = fflogsQueryConfig.abilityFilterSelected.filter(
      (v) => {
        return fflogsQueryConfig.abilityFilterCandidate.find((e) => e.actionId === v);
      },
    );
    isLoading.value = false;
  } catch (e) {
  ...
  async function queryEnemies(startTime: number, index: number) {
    if (index >= 0) {
      try {
        ...
        if (index < fflogsQueryConfig.bossIDs.length - 1) await queryEnemies(startTime, index + 1);
      } catch (e) {
        ElMessageBox.alert(
          `请稍后重试。如果问题持续存在,请检查您的网络连接。错误详情: ${e}`,
          "获取敌方技能数据失败",
  ...
  fflogsQueryConfig.abilityFilterEventsAfterFilterRawTimeline =
    fflogsQueryConfig.abilityFilterEvents
      .map((item) => {
        ...
        if ((source?.type === "NPC" || /unknown/.test(actionName)) && !window) return null;
  ...
  try {
    ...
    fflogsQueryConfig.abilityFilterCandidate = fflogsQueryConfig.abilityFilterEvents
      .reduce((total, item) => {
        if (
          ...
          !total.some((v) => v.actionName === item.actionName)
        ) {
          total.push(item);
        }
        return total;
    ...
    fflogsQueryConfig.enemiesAbilityFilterSelected =
      fflogsQueryConfig.enemiesAbilityFilterSelected.filter((v) => {
        return fflogsQueryConfig.abilityFilterCandidate.find((e) => e.actionName === v);
      });
    isLoading.value = false;
  } catch (e) {
    ElMessageBox.alert(
...
async function queryFFlogsReportEventsEnemies() {
  ...
  await queryEnemies(fflogsQueryConfig.start, 0);
...
function handeleFFlogsQueryResultEnemiesListFilter() {
  ...
  // )
  fflogsQueryConfig.abilityFilterEvents = fflogsQueryConfig.abilityFilterEvents
    .filter((event) => fflogsQueryConfig.enemiesAbilityFilterSelected.includes(event.actionName))
    .sort((a, b) => a.time - b.time);
  fflogsQueryConfig.abilityFilterEventsAfterFilterRawTimeline =
    fflogsQueryConfig.abilityFilterEvents
      .map((item) => {
        ...
        const regexType = type === "begincast" ? "StartsUsing" : "Ability";
        const hexId = actionId.toString(16).toUpperCase();
        const source = fflogsQueryConfig.enemies.find((e) => e.id === item.sourceID);
...
<template>
  <el-button
    :style="currentStep > 0 ? {} : { visibility: 'hidden', pointerEvent: 'none' }"
    type="primary"
    text
    position-absolute
    z-999
        ...
        >
          <template #append>
            <el-tooltip
              ...
              effect="light"
            >
              <el-button type="primary" link @click="openFFLogsProfile"> 如何获取 </el-button>
            </el-tooltip>
          </template>
        </el-input>
      </el-form-item>
  ...
  <div class="query-results">
    ...
    <el-card v-if="currentStep === 2" class="ability-filter">
      <div class="ability-filter-container">
        ...
        <template v-if="confirmEnabled && fflogsQueryConfig.type === 'friendies'">
          <el-select
          ...
          <el-form-item label="添加 TTS 语法" m-t-3 w-60>
        ...
        <template v-else-if="confirmEnabled && fflogsQueryConfig.type === 'enemies'">
          <el-select
            v-if="confirmEnabled"
