...
export function normalizeLegacySheet(sheet: SheetState): SheetState {
  ...
  const normalizedActions = (sheet.planner.playerActions || [])
    .map((action) => {
  ...
  return {
    ...
    mechanics: {
      ...
      filterMechanics:
        sheet.mechanics.filterMechanics || buildDefaultFilters(sheet.mechanics.rows || []),
    },
    planner: {
...
export function toSheetStorageShards(list: SheetState[]): SheetStorageShards {
  ...
  list.forEach((sheet) => {
    ...
    sheetPlannerMap[sheet.id] = {
      ...
      columns: sheet.planner.columns.map((col) => ({
        ...col,
        skills: [...col.skills],
        hiddenSkillIds: [...(col.hiddenSkillIds || [])],
      })),
...
export function fromSheetStorageShards(shards: SheetStorageShards) {
  return shards.sheetOrder
    .map((id) => {
      ...
      if (!meta || !mechanics || !planner) return null;
