...
function getPersonalDamageForColumn(row: MitigationRow, col: ColumnDef) {
  ...
  if (details && details.length > 0) {
    ...
    if (targetsAll) return Math.max(matchedDamage, fallback);
...
function isRowTargetingAllParty(row: MitigationRow, partyColumns: ColumnDef[]) {
  ...
  const targetNames = new Set(
    (row.targets || []).map((target) => String(target || "").trim()).filter(Boolean),
  );
  if (targetNames.has("全部")) return true;
  ...
  for (const col of partyColumns) {
    ...
    if (
      (colTargetId && combinedIds.has(colTargetId)) ||
      combinedIds.has(normalizeTargetId(col.key))
    ) {
      matched += 1;
      continue;
    }
    const candidates = getColumnTargetNameCandidates(col);
    if (candidates.size > 0 && [...candidates].some((name) => targetNames.has(name))) {
      matched += 1;
      continue;
    }
  }
...
function computeUseShieldSimulation(
  ...
  if (baseDuration <= 0) return { effectiveDuration: 0, absorbedByRowIndex };
  ...
  if (use.mitigationScope === "self") {
    if (!ownerCol) return { effectiveDuration: baseDuration, absorbedByRowIndex };
    let remainingShield = use.shieldAmount;
    for (let i = use.rowIndex; i < rows.length; i++) {
      const row = rows[i];
      if (!row) break;
      if (row.timestamp + EPSILON < use.timestamp) continue;
      if (row.timestamp >= endTimestamp - EPSILON) break;
  ...
  const partyColumns = [...columnByKey.values()];
  if (partyColumns.length === 0) return { effectiveDuration: baseDuration, absorbedByRowIndex };
  ...
  for (let i = use.rowIndex; i < rows.length; i++) {
    ...
    for (let index = 0; index < partyColumns.length; index++) {
      if (remainingShieldByMember[index]! <= EPSILON) continue;
      let consumed = getPersonalDamageForColumn(row, partyColumns[index]!);
      if (rowHitsAllParty) consumed = Math.max(consumed, fallbackDamage);
      if (consumed <= 0) continue;
      consumedMembers += 1;
...
export function useMitigationSimulator(
  ...
  function runSimulation() {
        ...
        if (matchedIndex !== undefined) {
          if (rowIndexByTimestamp === -1) {
            ...
            const keyTimeMatches =
              !!rowByKey && Math.abs(rowByKey.timestamp - action.timestamp) <= EPSILON;
            const tsTimeMatches =
              !!rowByTimestamp && Math.abs(rowByTimestamp.timestamp - action.timestamp) <= EPSILON;
    ...
    const rowShieldAbsorbedByIndex: number[] = Array.from({ length: rawRows.length }, () => 0);
    schedules.forEach((list, key) => {
      ...
      scheduleTimestampsByKey.set(
        key,
        list.map((use) => use.timestamp),
      );
    });
        ...
        col.skills.forEach((skill) => {
          ...
          if (startUse) {
            ...
            if (Math.abs(elapsed) <= EPSILON) {
              ...
              status = "active";
            } else if (elapsed >= activeDuration && elapsed < startUse.recast) {
              status = "cooldown";
            }
          }
          ...
          if (!status) {
            for (let i = 0; i < schedule.length; i++) {
              ...
              if (candidate && candidate.timestamp > row.timestamp) {
                ...
                if (timeToNext > 0 && timeToNext < requiredSpacing) status = "conflict";
                break;
              }
            }
          }
          ...
          if (status === "active" || status === "active-start") {
            const canApplyToRow =
              skill.mitigationScope !== "self" || getPersonalDamageForColumn(row, col) > EPSILON;
            if (canApplyToRow) activeMitigations.push(skill.id);
          }
          ...
          if (isChargeBased) {
            ...
            const chargeState = simulateChargeStateAtTime(
              scheduleTimestamps,
              row.timestamp,
              maxCharges,
              recast,
          ...
          if (status === "active" || status === "active-start") classes.push("is-active");
          if (status === "active-start") classes.push("is-start");
          if (status === "cooldown") classes.push("is-cooldown");
          if (status === "conflict") classes.push("is-pre-cooldown");

          const offset =
            status === "active-start" && currentMatchingUse
          ...
          if (status === "conflict" && nextUse) {
            ...
            recastLeft = Math.round(
              Math.max(
                0,
                currentMatchingUse.recast - (row.timestamp - currentMatchingUse.timestamp),
              ),
            );
          } else if (
            isChargeBased &&
            nextRechargeAtForCharges !== undefined &&
            charges < maxChargesCapacity
          ) {
