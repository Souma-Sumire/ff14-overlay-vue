...
function calculateReductionMultiplier(
  ...
  if (flagsEffect === "instant death" || flagsEffect === "dodge") return 0;

  let flagMultiplier = 1;
  if (flagsEffect === "blocked damage") flagMultiplier = 0.8;
  else if (flagsEffect === "parried damage") flagMultiplier = 0.85;
...
export function useLogParser() {
  ...
  function loadRsvData() {
    try {
      ...
      for (const [key, value] of Object.entries(parsed)) {
        const id = Number(key);
        if (id && value) rsvDataGlobal.set(id, value);
      }
    } catch (error) {
  ...
  function saveRsvData() {
    try {
      ...
      for (const [key, value] of rsvDataGlobal.entries()) stored[key] = value;
  ...
  async function parseFile(file: File): Promise<EncounterCandidate[]> {
    ...
    for (const line of lines) {
      ...
      if (type === logDefinitions.AddedCombatant.type) {
        ...
        if (combatant.id.startsWith("1")) allCombatants.set(combatant.id, combatant);
      }

      if (type === logDefinitions.PartyList.type) currentPartyIds = parsePartyIds(split);
      ...
      if (type === logDefinitions.InCombat.type) {
        ...
        if (isCombat && !inCombat && timestamp !== null) {
          ...
          inCombat = false;
          if (timestamp !== null) currentEncounter.endTime = timestamp;
          currentEncounter.durationStr = formatTime(
            (currentEncounter.endTime - currentEncounter.startTime) / 1000,
          );
          if (currentEncounter.lines.length > 20) parsedEncounters.push(currentEncounter);
      ...
      if (inCombat && currentEncounter) {
        ...
        if (lineTimestamp !== null) currentEncounter.endTime = lineTimestamp;

        if (type === logDefinitions.AddedCombatant.type) {
          const combatant = parseAddedCombatant(split);
          if (!combatant.id.startsWith("1")) continue;
  ...
  function extractMechanics(
    ...
    const pruneActiveCasts = (now: number) => {
      for (const [castKey, castData] of activeCasts.entries()) {
        if (now - castData.startedAt > CAST_CACHE_TTL_MS) activeCasts.delete(castKey);
      }
    };
    const resolveCastTime = (castKeys: string[], now: number) => {
      ...
      for (const castKey of castKeys) {
        ...
        if (age < 0 || age > CAST_MATCH_LOOKBACK_MS) continue;
        if (!bestMatch || age < bestMatch.age) bestMatch = { totalTime: castData.totalTime, age };
      }
    ...
    const getSanitizedNameById = (id: string, fallback: string) => {
      ...
      if (String(id || "").startsWith("1")) return "玩家";
    ...
    for (const line of encounter.lines) {
      ...
      if (type === logDefinitions.StartsUsing.type) {
        ...
        if (!Number.isFinite(castSeconds) || castSeconds < 1) continue;
      ...
      if (type === logDefinitions.GainsEffect.type) {
        ...
        if (targetId.startsWith("1") && keigenn) {
          if (!statusData[targetId]) statusData[targetId] = {};
      ...
      if (type === logDefinitions.LosesEffect.type) {
        ...
        if (statusData[targetId]) delete statusData[targetId][effectId];
        continue;
      }
      ...
      if (type === logDefinitions.StatusEffect.type) {
        ...
        const shieldValue =
          Number.parseInt(split[logDefinitions.StatusEffect.fields.currentShield] || "0", 10) || 0;
        if (targetId.startsWith("1")) shieldData[targetId] = shieldValue;
        continue;
      }
      ...
      let realPlayerId = sourceId.startsWith("1")
        ? sourceId
        : isOwnerPlayer
          ? abilityOwnerId
          : null;
      ...
      if (
        includePlayerActions &&
        realPlayerId &&
        partyIds.has(realPlayerId) &&
        validSkillIds.has(abilityIdDec)
      ) {
      ...
      if (sourceId.startsWith("1") || isOwnerPlayer || !includeMechanics) continue;

      const ability = processAbilityLine(split);
      if ((!ability.isAttack && ability.amount === 0) || ability.isHeal) continue;
      if (sourceId === targetId) continue;
      if (sourceId.startsWith("4") && targetId.startsWith("4") && sourceId !== targetId) continue;

      const flags = ability.flags;
      const { type: damageType, effect } = processFlags(flags);
      const reduction = calculateReductionMultiplier(
        damageType,
        effect,
        statusData[targetId] || {},
      );
      ...
      rawEvents.push({
        ...
        type: damageType,
        sourceId,
        sourceName: getSanitizedNameById(
          sourceId,
          split[logDefinitions.Ability.fields.source] || "Unknown",
        ),
        targetId,
        targetName: getSanitizedNameById(
          targetId,
          split[logDefinitions.Ability.fields.target] || "Unknown",
        ),
        flags,
        line,
        reduction,
    ...
    for (const event of sortedRawEvents) {
      ...
      const previousGroup = groupList.at(-1);
      if (
        previousGroup &&
        event.timestamp - previousGroup.firstTimestamp <= EVENT_GROUP_WINDOW_MS
      ) {
    ...
    for (let groupIndex = 0; groupIndex < sortedGroups.length; groupIndex += 1) {
      ...
      const normalizedDamages = group.events.map((event, index) => {
        ...
        if (member && Util.isTankJob(Util.jobEnumToJob(member.job))) raw = Math.round(raw / 0.8);
        return raw;
      });

      const simultaneousHitCount = Math.max(
        0,
        ...Array.from(targetsByTimestamp.values(), (bucket) => bucket.size),
      );
      ...
      const nonSpecialDamages = damagesForBaseline.filter(
        (damage) => Number.isFinite(damage) && damage > 0 && damage <= EXTREME_DAMAGE_THRESHOLD,
      );
      const baselinePool =
        nonSpecialDamages.length > 0
          ? nonSpecialDamages
      ...
      const filteredDamages = baselinePool.filter((damage) => {
        if (medianDamage <= 0) return true;
        return damage <= 2 * medianDamage;
      });
      const baselineCandidates = filteredDamages.length > 0 ? filteredDamages : baselinePool;
      const baselineRawDamage =
        baselineCandidates.length > 0 ? Math.round(calculateAverage(baselineCandidates)) : 0;
      ...
      const allTargetsHighDamage =
        estimatedRawDamages.length > 0
          ? estimatedRawDamages.every(
              (damage, index) => damage >= (targetThresholds[index] || 0) || damage === 0,
            )
          : false;
      const hasExtremeDamage = estimatedRawDamages.some(
        (damage) => damage > EXTREME_DAMAGE_THRESHOLD,
      );
      ...
      rowDrafts.push({
        ...
        flags,
        damageDetails,
        targets: [...uniqueTargetsById.values()],
        targetIds: [...uniqueTargetsById.keys()],
    ...
    rowDrafts.forEach((draft) => {
      ...
      if (draft.localIsAOE) state.isAOE = true;
      if (draft.localIsTB) state.isTB = true;
    ...
    const rows: MitigationRow[] = rowDrafts.map((draft) => {
      ...
      const state = actionTypeStateById.get(draft.actionId) || {
        isAOE: localIsAOE,
        isTB: localIsTB,
      };
      return {
