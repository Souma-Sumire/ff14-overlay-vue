...
export class WhiteMageTracker extends BaseTracker {
  ...
  public processLine(type: string, splitLine: string[]) {
    ...
    if (timestamp !== null) this.updateLilies(timestamp);
          ...
          for (const id of this.playerIds) {
            if (isCombat) {
              if (!this.inCombat[id]) {
                ...
                if (timestamp !== null) this.lastGenTime[id] = timestamp;
              }
      ...
      case "22":
        {
          ...
          if (!this.inCombat[sourceId]) {
            ...
            if (timestamp !== null) this.lastGenTime[sourceId] = timestamp;
          }
