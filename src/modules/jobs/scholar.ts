...
export class ScholarTracker extends BaseTracker {
  ...
  public isReady(characterId: string, skillId: number, cost: number): boolean {
    ...
    if (
      this.recitation[characterId] &&
      (skillId === SCH_ACTION_IDS.INDOMITABILITY || skillId === SCH_ACTION_IDS.EXCOGITATION)
    ) {
      return true;
    }
  ...
  public getExtraText(
    ...
    skillId: number,
    timestamp: number,
    _allCooldowns: Record<number, number[]>,
  ): string {
      ...
      case "26": // GainsEffect
        {
          ...
          if (!this.playerIds.has(targetId)) return;
          const effectId = Number.parseInt(
