...
export function cloneTeamWatchActionMetaMap(input: Record<number, TeamWatchActionMetaRaw>) {
  return Object.fromEntries(Object.entries(input).map(([k, v]) => [Number(k) || 0, { ...v }]));
}
...
export function buildInheritedBaseJobActions(baseJob: number, sourceActions: unknown): number[] {
  ...
  if (bj <= 0 || Util.baseJobEnumConverted(bj) === bj) return normalize(sourceActions);
  return normalize(sourceActions).map((aid) =>
    aid > 0 ? resolveHighestSupportedActionInFamily(aid, bj) : 0,
  );
}
...
export function normalizeTeamWatchActionMetaRaw(aid: number, value: any): TeamWatchActionMetaRaw {
  ...
  if (iconId <= 0 && raw.iconSrc && typeof raw.iconSrc === "string") {
    ...
    if (match) iconId = Number(match[1]);
  }
...
function normalizeWatchJobs(value: any): Record<number, number[]> {
  ...
  if (value && typeof value === "object") {
    Object.entries(value).forEach(([j, acts]) => {
      const jn = Number(j);
      if (jn > 0) base[jn] = (Array.isArray(acts) ? acts : []).map((v) => Number(v) || 0);
    });
  }
  return base;
}
...
function normalizeActionMeta(value: any): Record<number, TeamWatchActionMetaRaw> {
  ...
  if (value) {
    Object.entries(value).forEach(([id, m]) => {
      const n = Number(id);
      if (n > 0) res[n] = normalizeTeamWatchActionMetaRaw(n, m);
    });
  }
  return res;
}
...
export function loadTeamWatchStorageData(): TeamWatchStorageData {
  ...
  try {
    ...
    if (current) return buildData(JSON.parse(current));
  } catch {
    /* ignore */
  }
  return buildData();
