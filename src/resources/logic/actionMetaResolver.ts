...
function persistKnownRoleActionIds() {
  roleActionIdStorage.value = Object.fromEntries(
    Array.from(knownRoleActionIds, (id) => [String(id), 1 as const]),
  );
}
...
Object.values(BAKED_ACTION_META_LITE_BY_ID).forEach((meta) => {
  ...
  if (meta.isRoleAction || cjId <= 0 || cjcId <= 0) return;

  const resolvedJobs = battleJobEnums.filter(
    (j) =>
      j === cjId || Util.baseJobEnumConverted(j) === cjId || Util.baseJobEnumConverted(cjId) === j,
  );
  if (!resolvedJobs.length) return;

  if (!jobsByClassJobCategory.has(cjcId)) jobsByClassJobCategory.set(cjcId, []);
  const targetJobs = jobsByClassJobCategory.get(cjcId)!;
  resolvedJobs.forEach((j) => {
    if (!targetJobs.includes(j)) targetJobs.push(j);
    if (!categoriesByJob.has(j)) categoriesByJob.set(j, []);
    const jobCats = categoriesByJob.get(j)!;
    if (!jobCats.includes(cjcId)) jobCats.push(cjcId);
  });
});
...
export function resolveActionMinLevel(
  level: any,
  options?: { actionId?: number; isRoleAction?: any; fallback?: number },
) {
...
export function resolveActionJobsFromTargets(cj: number, cjc: number, ac: number, isRole: boolean) {
  if (isRole) {
    ...
    return battleJobEnums.filter((j) =>
      cats.some((c) => (ROLE_ACTION_CATEGORY_BY_JOB[j] ?? []).includes(c)),
    );
  }
  if (cj <= 0) return [...(jobsByClassJobCategory.get(cjc) ?? [])];
  return battleJobEnums.filter(
    (j) => j === cj || Util.baseJobEnumConverted(j) === cj || Util.baseJobEnumConverted(cj) === j,
  );
}
...
export function resolveBakedActionMeta(actionId: number): ResolvedBakedActionMeta | undefined {
  ...
  return {
    ...
    classJobLevel: resolveActionMinLevel(baked.classJobLevel || 1, {
      actionId,
      isRoleAction: isRole,
    }),
    isRoleAction: isRole,
...
export function initGlobalSkills(definitions: any[]) {
  ...
  definitions.forEach((d) => {
    ...
    if (j.length) jobsBySkillId.set(id, uniqueInts([...(jobsBySkillId.get(id) ?? []), ...j]));
  });
...
export function resolveActionMetaByLevel(
  ...
  const rRecast =
    Number(parseDynamicValue(global?.recast1000ms ?? options?.baseRecast1000ms ?? 0, level)) ||
    (options?.baseRecast1000ms ?? 0);
  const rDur =
    Number(parseDynamicValue(global?.duration ?? options?.baseDuration ?? 0, level)) ||
    (options?.baseDuration ?? 0);
  ...
  return {
    resolvedId: rId,
    recast1000ms: rRecast,
    duration: rDur,
    minLevel: minL,
    jobsFromGlobal: global?.job ?? [],
  };
}
...
export function resolveApiActionMeta(actionId: number, row: any) {
  ...
  if (isRole) markRoleActionId(id, 1);
  ...
  if (Array.isArray(row._ClassJobCategoryJobs) && row._ClassJobCategoryJobs.length > 0) {
    if (isRole) {
      ...
      if (cj > 0)
        jobs = battleJobEnums.filter(
          (j) =>
            j === cj || Util.baseJobEnumConverted(j) === cj || Util.baseJobEnumConverted(cj) === j,
        );
      else jobs = battleJobEnums.filter((j) => row._ClassJobCategoryJobs.includes(j));
    }
  } else {
    jobs = resolveActionJobsFromTargets(cj, cjc, ac, isRole);
  }
  ...
  return {
    ...
    classJobLevel: resolveActionMinLevel(row.ClassJobLevel || 1, {
      actionId: id,
      isRoleAction: isRole,
    }),
    isRoleAction: isRole,
...
export function markRoleActionId(actionId: number, isRoleAction: any) {
  ...
  if (id <= 0 || (Number(isRoleAction) || 0) <= 0) return;
...
export function searchActionNamesLite(
  query: string,
  limit = 100,
  ...
  if (!cachedActionList) {
    cachedActionList = Object.entries(BAKED_ACTION_META_LITE_BY_ID)
      .map(([id, row]) => {
        ...
        if (!name || !actionId2ClassJobLevel(nid)) return null;
        return { id: nid, name };
      })
...
export function resolveActionIconSrc(aid: number, options?: { apiIconPath?: string }) {
  ...
  if (iconId > 0) return getIconSrcById(iconId);
...
function getUpgradeFamily(actionId: number) {
  ...
  if (top <= 0) return [];
  if (upgradeFamilyCache.has(top)) return upgradeFamilyCache.get(top)!;
...
function getUpgradeDepth(actionId: number) {
  if (actionId <= 0) return 999;
  if (upgradeDepthCache.has(actionId)) return upgradeDepthCache.get(actionId)!;
  let depth = 0;
  let curr = actionId;
...
export function resolveHighestSupportedActionInFamily(actionId: number, baseJob: number) {
  ...
  for (const cid of family) {
    ...
    if (
      !baked ||
      (baked.isRoleAction
        ? !baked.jobs.includes(baseJob)
        : baked.classJobTargetId > 0 && baked.classJobTargetId !== baseJob)
    )
      continue;
    ...
    if (
      lv > bestLv ||
      (lv === bestLv && depth < bestDepth) ||
      (lv === bestLv && depth === bestDepth && cid === actionId)
    ) {
      bestId = cid;
      bestLv = lv;
      bestDepth = depth;
