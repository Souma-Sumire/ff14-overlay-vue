import type {
  MitigationMechanicsImportData,
  MitigationPlayerActionsImportData,
} from "@/composables/mitigation/useMitigationImportCodec";

export interface MitigationSheetTemplate {
  id: string;
  name: string;
  description?: string;
  mechanics?: MitigationMechanicsImportData;
  playerActions?: MitigationPlayerActionsImportData;
  map: {
    zoneId: number;
  };
}

export const mitigationSheetTemplates: MitigationSheetTemplate[] = [
  {
    id: "m1s",
    name: "M1S - 模板",
    description: "由 M1S - 模板 生成",
    mechanics: {
      type: "mitigation-mechanics",
      actions: [
        "Prepull",
        "平A",
        "魅亡之音",
        "硬核之声",
        "共振波",
        "粗暴之雨",
        "施虐的尖啸",
        "月之半相",
        "全场杀伤",
        "致命的闭幕曲",
        "以太流失",
        "贪欲无厌",
        "掉落",
        "血锁牢狱",
        "音速流散",
        "音速集聚",
      ],
      rows: [
        {
          t: -30,
          i: "",
          n: 0,
          v: 0,
          ty: 0,
          f: 0,
        },
        {
          t: -25,
          i: "",
          n: 0,
          v: 0,
          ty: 0,
          f: 0,
        },
        {
          t: -20,
          i: "",
          n: 0,
          v: 0,
          ty: 0,
          f: 0,
        },
        {
          t: -15,
          i: "",
          n: 0,
          v: 0,
          ty: 0,
          f: 0,
        },
        {
          t: -10,
          i: "",
          n: 0,
          v: 0,
          ty: 0,
          f: 0,
        },
        {
          t: -5,
          i: "",
          n: 0,
          v: 0,
          ty: 0,
          f: 0,
        },
        {
          t: 0,
          i: "",
          n: 0,
          v: 0,
          ty: 0,
          f: 0,
        },
        {
          t: 4.02,
          i: "BBA7",
          n: 1,
          v: 70000,
          ty: 0,
          f: 0,
          tg: ["骑"],
        },
        {
          t: 11.16,
          i: "B384",
          n: 2,
          v: 170000,
          ty: 1,
          f: 1,
          tc: 8,
          ct: 4.7,
          tg: ["全部"],
        },
        {
          t: 12.09,
          i: "BBA7",
          n: 1,
          v: 70000,
          ty: 0,
          f: 0,
          tg: ["骑"],
        },
        {
          t: 15.13,
          i: "BBA7",
          n: 1,
          v: 70000,
          ty: 0,
          f: 0,
          tg: ["骑"],
        },
        {
          t: 21.32,
          i: "B37F",
          n: 3,
          v: 400000,
          ty: 1,
          f: 2,
          tc: 2,
          ct: 4.7,
          tg: ["骑", "暗"],
        },
        {
          t: 23.37,
          i: "BBA7",
          n: 1,
          v: 70000,
          ty: 0,
          f: 0,
          tg: ["骑"],
        },
        {
          t: 33.53,
          i: "BBA7",
          n: 1,
          v: 70000,
          ty: 0,
          f: 0,
          tg: ["骑"],
        },
        {
          t: 36.38,
          i: "B376",
          n: 4,
          v: 100000,
          ty: 1,
          f: 0,
          tc: 4,
          tg: ["骑", "暗", "武", "镰"],
        },
        {
          t: 36.56,
          i: "BBA7",
          n: 1,
          v: 70000,
          ty: 0,
          f: 0,
          tg: ["骑"],
        },
        {
          t: 39.59,
          i: "BBA7",
          n: 1,
          v: 70000,
          ty: 0,
          f: 0,
          tg: ["骑"],
        },
        {
          t: 39.77,
          i: "B376",
          n: 4,
          v: 100000,
          ty: 1,
          f: 0,
          tc: 4,
          tg: ["白", "学", "舞", "黑"],
        },
        {
          t: 47.7,
          i: "B383",
          n: 5,
          v: 95000,
          ty: 1,
          f: 4,
          tc: 8,
          tg: ["全部"],
        },
        {
          t: 48.76,
          i: "B383",
          n: 5,
          v: 95000,
          ty: 1,
          f: 4,
          tc: 8,
          tg: ["全部"],
        },
        {
          t: 49.7,
          i: "BBA7",
          n: 1,
          v: 70000,
          ty: 0,
          f: 0,
          tg: ["骑"],
        },
        {
          t: 49.83,
          i: "B383",
          n: 5,
          v: 95000,
          ty: 1,
          f: 4,
          tc: 8,
          tg: ["全部"],
        },
        {
          t: 52.73,
          i: "BBA7",
          n: 1,
          v: 70000,
          ty: 0,
          f: 0,
          tg: ["骑"],
        },
        {
          t: 61.91,
          i: "B366",
          n: 6,
          v: 200000,
          ty: 1,
          f: 1,
          tc: 8,
          tg: ["全部"],
        },
        {
          t: 63.07,
          i: "BBA7",
          n: 1,
          v: 70000,
          ty: 0,
          f: 0,
          tg: ["骑"],
        },
        {
          t: 66.11,
          i: "BBA7",
          n: 1,
          v: 70000,
          ty: 0,
          f: 0,
          tg: ["骑"],
        },
        {
          t: 76.4,
          i: "B37B",
          n: 7,
          v: 0,
          ty: 1,
          f: 0,
          ct: 4.7,
        },
        {
          t: 126.36,
          i: "B7C8",
          n: 1,
          v: 70000,
          ty: 1,
          f: 0,
          tg: ["骑"],
        },
        {
          t: 129.39,
          i: "B7C8",
          n: 1,
          v: 70000,
          ty: 1,
          f: 0,
          tg: ["骑"],
        },
        {
          t: 132.42,
          i: "B7C8",
          n: 1,
          v: 70000,
          ty: 1,
          f: 0,
          tg: ["骑"],
        },
        {
          t: 139.38,
          i: "B366",
          n: 6,
          v: 200000,
          ty: 1,
          f: 1,
          tc: 8,
          tg: ["全部"],
        },
        {
          t: 140.49,
          i: "B7C8",
          n: 1,
          v: 70000,
          ty: 1,
          f: 0,
          tg: ["骑"],
        },
        {
          t: 143.52,
          i: "BBA7",
          n: 1,
          v: 70000,
          ty: 0,
          f: 0,
          tg: ["骑"],
        },
        {
          t: 153.82,
          i: "B36D",
          n: 8,
          v: 200000,
          ty: 1,
          f: 1,
          tc: 8,
          tg: ["全部"],
        },
        {
          t: 156.31,
          i: "BBA7",
          n: 1,
          v: 70000,
          ty: 0,
          f: 0,
          tg: ["骑"],
        },
        {
          t: 159.34,
          i: "BBA7",
          n: 1,
          v: 70000,
          ty: 0,
          f: 0,
          tg: ["骑"],
        },
        {
          t: 162.38,
          i: "BBA7",
          n: 1,
          v: 70000,
          ty: 0,
          f: 0,
          tg: ["骑"],
        },
        {
          t: 172.8,
          i: "B36F",
          n: 9,
          v: 200000,
          ty: 1,
          f: 1,
          tc: 8,
          tg: ["全部"],
        },
        {
          t: 174.72,
          i: "B7C8",
          n: 1,
          v: 70000,
          ty: 1,
          f: 0,
          tg: ["骑"],
        },
        {
          t: 177.75,
          i: "B7C8",
          n: 1,
          v: 70000,
          ty: 1,
          f: 0,
          tg: ["骑"],
        },
        {
          t: 193.84,
          i: "B392",
          n: 10,
          v: 140000,
          ty: 1,
          f: 1,
          tc: 8,
          ct: 4.7,
          tg: ["全部"],
        },
        {
          t: 199.05,
          i: "B7C8",
          n: 1,
          v: 70000,
          ty: 1,
          f: 0,
          tg: ["骑"],
        },
        {
          t: 202.08,
          i: "B7C8",
          n: 1,
          v: 70000,
          ty: 1,
          f: 0,
          tg: ["骑"],
        },
        {
          t: 205.11,
          i: "B7C8",
          n: 1,
          v: 70000,
          ty: 1,
          f: 0,
          tg: ["骑"],
        },
        {
          t: 208.14,
          i: "B7C8",
          n: 1,
          v: 70000,
          ty: 1,
          f: 0,
          tg: ["骑"],
        },
        {
          t: 211.17,
          i: "B7C8",
          n: 1,
          v: 70000,
          ty: 1,
          f: 0,
          tg: ["骑"],
        },
        {
          t: 214.2,
          i: "B7C8",
          n: 1,
          v: 70000,
          ty: 1,
          f: 0,
          tg: ["骑"],
        },
        {
          t: 221.24,
          i: "B37F",
          n: 3,
          v: 400000,
          ty: 1,
          f: 2,
          tc: 2,
          ct: 4.7,
          tg: ["骑", "暗"],
        },
        {
          t: 223.29,
          i: "BBA7",
          n: 1,
          v: 70000,
          ty: 0,
          f: 0,
          tg: ["骑"],
        },
        {
          t: 234.52,
          i: "BBA7",
          n: 1,
          v: 70000,
          ty: 0,
          f: 0,
          tg: ["暗"],
        },
        {
          t: 236.3,
          i: "B376",
          n: 4,
          v: 100000,
          ty: 1,
          f: 0,
          tc: 4,
          tg: ["骑", "暗", "武", "镰"],
        },
        {
          t: 237.55,
          i: "BBA7",
          n: 1,
          v: 70000,
          ty: 0,
          f: 0,
          tg: ["暗"],
        },
        {
          t: 240.58,
          i: "B376",
          n: 4,
          v: 100000,
          ty: 1,
          f: 0,
          tc: 4,
          tg: ["白", "学", "舞", "黑"],
        },
        {
          t: 240.58,
          i: "BBA7",
          n: 1,
          v: 70000,
          ty: 0,
          f: 0,
          tg: ["暗"],
        },
        {
          t: 257.75,
          i: "B383",
          n: 5,
          v: 95000,
          ty: 1,
          f: 4,
          tc: 8,
          tg: ["全部"],
        },
        {
          t: 258.82,
          i: "B383",
          n: 5,
          v: 95000,
          ty: 1,
          f: 4,
          tc: 8,
          tg: ["全部"],
        },
        {
          t: 259.75,
          i: "BBA7",
          n: 1,
          v: 70000,
          ty: 0,
          f: 0,
          tg: ["暗"],
        },
        {
          t: 259.88,
          i: "B383",
          n: 5,
          v: 95000,
          ty: 1,
          f: 4,
          tc: 8,
          tg: ["全部"],
        },
        {
          t: 260.95,
          i: "B383",
          n: 5,
          v: 95000,
          ty: 1,
          f: 4,
          tc: 8,
          tg: ["全部"],
        },
        {
          t: 262.77,
          i: "BBA7",
          n: 1,
          v: 70000,
          ty: 0,
          f: 0,
          tg: ["暗"],
        },
        {
          t: 271.91,
          i: "B372",
          n: 11,
          v: 150000,
          ty: 1,
          f: 1,
          tc: 8,
          tg: ["全部"],
        },
        {
          t: 273.87,
          i: "BBA7",
          n: 1,
          v: 70000,
          ty: 0,
          f: 0,
          tg: ["骑"],
        },
        {
          t: 285.05,
          i: "B366",
          n: 6,
          v: 200000,
          ty: 1,
          f: 1,
          tc: 8,
          tg: ["全部"],
        },
        {
          t: 286.21,
          i: "B7C8",
          n: 1,
          v: 70000,
          ty: 1,
          f: 0,
          tg: ["骑"],
        },
        {
          t: 289.25,
          i: "B7C8",
          n: 1,
          v: 70000,
          ty: 1,
          f: 0,
          tg: ["骑"],
        },
        {
          t: 292.27,
          i: "B7C8",
          n: 1,
          v: 70000,
          ty: 1,
          f: 0,
          tg: ["骑"],
        },
        {
          t: 295.3,
          i: "B7C8",
          n: 1,
          v: 70000,
          ty: 1,
          f: 0,
          tg: ["骑"],
        },
        {
          t: 297.3,
          i: "B38B",
          n: 12,
          v: 150000,
          ty: 0,
          f: 2,
          tc: 2,
          ct: 6.7,
          tg: ["骑", "暗"],
        },
        {
          t: 298.32,
          i: "B7C8",
          n: 1,
          v: 70000,
          ty: 1,
          f: 0,
          tg: ["骑"],
        },
        {
          t: 306.35,
          i: "B384",
          n: 2,
          v: 170000,
          ty: 1,
          f: 1,
          tc: 8,
          ct: 4.7,
          tg: ["全部"],
        },
        {
          t: 306.39,
          i: "B7C8",
          n: 1,
          v: 70000,
          ty: 1,
          f: 0,
          tg: ["骑"],
        },
        {
          t: 309.42,
          i: "B7C8",
          n: 1,
          v: 70000,
          ty: 1,
          f: 0,
          tg: ["骑"],
        },
        {
          t: 312.44,
          i: "B7C8",
          n: 1,
          v: 70000,
          ty: 1,
          f: 0,
          tg: ["骑"],
        },
        {
          t: 315.47,
          i: "B7C8",
          n: 1,
          v: 70000,
          ty: 1,
          f: 0,
          tg: ["骑"],
        },
        {
          t: 315.47,
          i: "B38B",
          n: 12,
          v: 150000,
          ty: 0,
          f: 2,
          tc: 2,
          ct: 6.7,
          tg: ["骑", "暗"],
        },
        {
          t: 318.5,
          i: "B7C8",
          n: 1,
          v: 70000,
          ty: 1,
          f: 0,
          tg: ["骑"],
        },
        {
          t: 324.52,
          i: "B384",
          n: 2,
          v: 170000,
          ty: 1,
          f: 1,
          tc: 8,
          ct: 4.7,
          tg: ["全部"],
        },
        {
          t: 326.57,
          i: "B7C8",
          n: 1,
          v: 70000,
          ty: 1,
          f: 0,
          tg: ["骑"],
        },
        {
          t: 329.6,
          i: "B7C8",
          n: 1,
          v: 70000,
          ty: 1,
          f: 0,
          tg: ["骑"],
        },
        {
          t: 332.63,
          i: "B7C8",
          n: 1,
          v: 70000,
          ty: 1,
          f: 0,
          tg: ["骑"],
        },
        {
          t: 333.65,
          i: "B38B",
          n: 12,
          v: 150000,
          ty: 0,
          f: 2,
          tc: 2,
          ct: 6.7,
          tg: ["骑", "暗"],
        },
        {
          t: 335.66,
          i: "B7C8",
          n: 1,
          v: 70000,
          ty: 1,
          f: 0,
          tg: ["骑"],
        },
        {
          t: 338.68,
          i: "B7C8",
          n: 1,
          v: 70000,
          ty: 1,
          f: 0,
          tg: ["骑"],
        },
        {
          t: 341.71,
          i: "B7C8",
          n: 1,
          v: 70000,
          ty: 1,
          f: 0,
          tg: ["骑"],
        },
        {
          t: 344.75,
          i: "B7C8",
          n: 1,
          v: 70000,
          ty: 1,
          f: 0,
          tg: ["骑"],
        },
        {
          t: 347.77,
          i: "B7C8",
          n: 1,
          v: 70000,
          ty: 1,
          f: 0,
          tg: ["骑"],
        },
        {
          t: 350.81,
          i: "B7C8",
          n: 1,
          v: 70000,
          ty: 1,
          f: 0,
          tg: ["骑"],
        },
        {
          t: 357.58,
          i: "B366",
          n: 6,
          v: 200000,
          ty: 1,
          f: 1,
          tc: 8,
          tg: ["全部"],
        },
        {
          t: 358.87,
          i: "BBA7",
          n: 1,
          v: 70000,
          ty: 0,
          f: 0,
          tg: ["骑"],
        },
        {
          t: 361.91,
          i: "BBA7",
          n: 1,
          v: 70000,
          ty: 0,
          f: 0,
          tg: ["骑"],
        },
        {
          t: 372.02,
          i: "B36D",
          n: 8,
          v: 200000,
          ty: 1,
          f: 1,
          tc: 8,
          tg: ["全部"],
        },
        {
          t: 374.51,
          i: "BBA7",
          n: 1,
          v: 70000,
          ty: 0,
          f: 0,
          tg: ["骑"],
        },
        {
          t: 377.55,
          i: "BBA7",
          n: 1,
          v: 70000,
          ty: 0,
          f: 0,
          tg: ["骑"],
        },
        {
          t: 380.57,
          i: "BBA7",
          n: 1,
          v: 70000,
          ty: 0,
          f: 0,
          tg: ["骑"],
        },
        {
          t: 391,
          i: "B370",
          n: 9,
          v: 220000,
          ty: 1,
          f: 1,
          tc: 8,
          tg: ["全部"],
        },
        {
          t: 392.92,
          i: "B7C8",
          n: 1,
          v: 70000,
          ty: 1,
          f: 0,
          tg: ["骑"],
        },
        {
          t: 395.95,
          i: "B7C8",
          n: 1,
          v: 70000,
          ty: 1,
          f: 0,
          tg: ["骑"],
        },
        {
          t: 402.01,
          i: "B396",
          n: 13,
          v: 40000,
          ty: 1,
          f: 1,
          tc: 8,
          ct: 4.7,
          tg: ["全部"],
        },
        {
          t: 409.9,
          i: "B883",
          n: 14,
          v: 150000,
          ty: 1,
          f: 2,
          tg: ["暗"],
        },
        {
          t: 417.03,
          i: "B39F",
          n: 15,
          v: 150000,
          ty: 1,
          f: 4,
          tc: 8,
          tg: ["全部"],
        },
        {
          t: 424.42,
          i: "B396",
          n: 13,
          v: 40000,
          ty: 1,
          f: 1,
          tc: 8,
          ct: 4.7,
          tg: ["全部"],
        },
        {
          t: 432.31,
          i: "B883",
          n: 14,
          v: 150000,
          ty: 1,
          f: 2,
          tg: ["骑"],
        },
        {
          t: 439.45,
          i: "B39F",
          n: 15,
          v: 150000,
          ty: 1,
          f: 4,
          tc: 8,
          tg: ["全部"],
        },
        {
          t: 441.28,
          i: "B7C8",
          n: 1,
          v: 70000,
          ty: 1,
          f: 0,
          tg: ["骑"],
        },
        {
          t: 444.31,
          i: "B7C8",
          n: 1,
          v: 70000,
          ty: 1,
          f: 0,
          tg: ["骑"],
        },
        {
          t: 447.35,
          i: "B7C8",
          n: 1,
          v: 70000,
          ty: 1,
          f: 0,
          tg: ["骑"],
        },
        {
          t: 454.87,
          i: "B3A1",
          n: 13,
          v: 160000,
          ty: 1,
          f: 4,
          tc: 8,
          ct: 4.7,
          tg: ["全部"],
        },
        {
          t: 456.92,
          i: "B7C8",
          n: 1,
          v: 70000,
          ty: 1,
          f: 0,
          tg: ["骑"],
        },
        {
          t: 473.27,
          i: "B7C8",
          n: 1,
          v: 70000,
          ty: 1,
          f: 0,
          tg: ["骑"],
        },
        {
          t: 476.3,
          i: "B7C8",
          n: 1,
          v: 70000,
          ty: 1,
          f: 0,
          tg: ["骑"],
        },
        {
          t: 491.63,
          i: "B7C8",
          n: 1,
          v: 70000,
          ty: 1,
          f: 0,
          tg: ["骑"],
        },
        {
          t: 499.79,
          i: "B383",
          n: 5,
          v: 95000,
          ty: 1,
          f: 4,
          tc: 8,
          tg: ["全部"],
        },
        {
          t: 500.86,
          i: "B383",
          n: 5,
          v: 95000,
          ty: 1,
          f: 4,
          tc: 8,
          tg: ["全部"],
        },
        {
          t: 501.79,
          i: "BBA7",
          n: 1,
          v: 70000,
          ty: 0,
          f: 0,
          tg: ["骑"],
        },
        {
          t: 501.93,
          i: "B383",
          n: 5,
          v: 95000,
          ty: 1,
          f: 4,
          tc: 8,
          tg: ["全部"],
        },
        {
          t: 503,
          i: "B383",
          n: 5,
          v: 95000,
          ty: 1,
          f: 4,
          tc: 8,
          tg: ["全部"],
        },
        {
          t: 504.06,
          i: "B383",
          n: 5,
          v: 95000,
          ty: 1,
          f: 4,
          tc: 8,
          tg: ["全部"],
        },
        {
          t: 504.82,
          i: "BBA7",
          n: 1,
          v: 70000,
          ty: 0,
          f: 0,
          tg: ["骑"],
        },
        {
          t: 507.85,
          i: "BBA7",
          n: 1,
          v: 70000,
          ty: 0,
          f: 0,
          tg: ["骑"],
        },
        {
          t: 514.45,
          i: "B376",
          n: 4,
          v: 100000,
          ty: 1,
          f: 0,
          tc: 4,
          tg: ["骑", "暗", "武", "镰"],
        },
        {
          t: 515.02,
          i: "BBA7",
          n: 1,
          v: 70000,
          ty: 0,
          f: 0,
          tg: ["骑"],
        },
        {
          t: 518.06,
          i: "BBA7",
          n: 1,
          v: 70000,
          ty: 0,
          f: 0,
          tg: ["骑"],
        },
        {
          t: 521.09,
          i: "BBA7",
          n: 1,
          v: 70000,
          ty: 0,
          f: 0,
          tg: ["骑"],
        },
        {
          t: 521.18,
          i: "B376",
          n: 4,
          v: 100000,
          ty: 1,
          f: 0,
          tc: 4,
          tg: ["白", "学", "舞", "黑"],
        },
        {
          t: 529.11,
          i: "B379",
          n: 7,
          v: 0,
          ty: 1,
          f: 0,
          ct: 4.7,
        },
        {
          t: 539.23,
          i: "B380",
          n: 3,
          v: 560000,
          ty: 1,
          f: 2,
          tc: 2,
          ct: 4.7,
          tg: ["骑", "暗"],
        },
        {
          t: 542.3,
          i: "BBA7",
          n: 1,
          v: 70000,
          ty: 0,
          f: 0,
          tg: ["骑"],
        },
      ],
      aliases: {
        B405: "AA",
        BBA7: "平A",
        B7C8: "平A",
      },
    },
    playerActions: {
      type: "mitigation-player-actions",
      src: ["19_1", "32_1", "24_1", "28_1", "34_1", "39_1", "38_1", "25_1"],
      data: {},
      cols: [
        {
          i: "19_1",
          j: 19,
          h: [7382, 7548, 25746],
        },
        {
          i: "32_1",
          j: 32,
          h: [7548],
        },
        {
          i: "24_1",
          j: 24,
          h: [],
        },
        {
          i: "28_1",
          j: 28,
          h: [7561, 7434, 25867, 7559, 16538],
        },
        {
          i: "34_1",
          j: 34,
          h: [7542, 7541, 7548, 36962],
        },
        {
          i: "39_1",
          j: 39,
          h: [24404, 7542, 7541, 7548],
        },
        {
          i: "38_1",
          j: 38,
          h: [7541, 7548],
        },
        {
          i: "25_1",
          j: 25,
          h: [155, 7559, 157],
        },
      ],
    },
    map: {
      zoneId: 1321,
    },
  },
];
